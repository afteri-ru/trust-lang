// test/integration/playground_main.cpp
...
int main(int argc, const char* argv[]) {
    if (argc < 5) {
        std::cerr << "usage: integration <trust-playground> <trust-lsp> <examples-dir> <tmp-dir>\n";
        return 2;
    }
    ...
    if (g_failures != 0) {
        std::cerr << "integration: " << g_failures << " failure(s)\n";
        return 1;
    }
    std::cerr << "integration: OK (server->worker->lsp->worker->server, " << files.size() << " files, concurrent)" << "\n";
    return 0;
}
