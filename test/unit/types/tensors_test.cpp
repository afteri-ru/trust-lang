...
TEST_F(TensorTest, PluginAutoLoaded) {
    // Returns true when the plugin is found, false otherwise (no exception thrown).
    bool loaded = ensure_tensor_runtime_loaded();
    EXPECT_TRUE(loaded) << "libtensor_cpu.so not found";
}
