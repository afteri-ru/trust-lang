...
TEST(FormatterTest, ConfigParseKeywordSigil) {
    // По умолчанию — Remove (убирать '@').
    FormatConfig def = parseConfig("");
    ASSERT_TRUE(def.ok) << def.error;
    EXPECT_EQ(def.opts.keyword_sigil, KeywordSigil::Remove);
    // Явные режимы remove | add | none.
    FormatConfig c = parseConfig("KeywordSigil: add\n");
    ASSERT_TRUE(c.ok) << c.error;
    EXPECT_EQ(c.opts.keyword_sigil, KeywordSigil::Add);
    c = parseConfig("KeywordSigil: none\n");
    ASSERT_TRUE(c.ok) << c.error;
    EXPECT_EQ(c.opts.keyword_sigil, KeywordSigil::None);
    c = parseConfig("KeywordSigil: remove\n");
    ASSERT_TRUE(c.ok) << c.error;
    EXPECT_EQ(c.opts.keyword_sigil, KeywordSigil::Remove);
    // Невалидный режим — ошибка (no silent fallback).
    c = parseConfig("KeywordSigil: bogus\n");
    EXPECT_FALSE(c.ok);
    EXPECT_FALSE(c.error.empty());
}

TEST(FormatterTest, KeywordSigilStringifyRoundTrip) {
    // X-macro: строки генерируются из KEYWORD_SIGIL_LIST — toString/fromString согласованы.
    for (KeywordSigil ks : {KeywordSigil::Remove, KeywordSigil::Add, KeywordSigil::None}) {
        const std::string s = std::string(keywordSigilToString(ks));
        EXPECT_FALSE(s.empty());
        KeywordSigil back;
        EXPECT_TRUE(keywordSigilFromString(s, back));
        EXPECT_EQ(back, ks);
    }
    // Известные строковые имена.
    EXPECT_EQ(keywordSigilToString(KeywordSigil::Remove), "remove");
    EXPECT_EQ(keywordSigilToString(KeywordSigil::Add), "add");
    EXPECT_EQ(keywordSigilToString(KeywordSigil::None), "none");
    // Неизвестное значение — false (no silent fallback).
    KeywordSigil out = KeywordSigil::Remove;
    EXPECT_FALSE(keywordSigilFromString("bogus", out));
}
...
TEST(FormatterTest, DumpConfigRoundTrip) {
    ...
    EXPECT_NE(dump.find("KeywordSigil: remove"), std::string::npos);
