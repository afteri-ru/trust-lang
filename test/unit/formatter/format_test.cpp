...
TEST(FormatterTest, KeywordSigilRemoveDefault) {
    // По умолчанию KeywordSigil::Remove — убрать '@' у ключевого слова (if — из DSL keywords).
    const std::string in = "@if(x){ y:=1; }";
    const std::string exp = "if (x) {\n    y := 1;\n}\n";
    EXPECT_EQ(fmt(in), exp);
}

TEST(FormatterTest, KeywordSigilAdd) {
    FormatOptions opts;
    opts.keywords = {"if"};
    opts.keyword_sigil = KeywordSigil::Add;
    // Без '@' в исходнике → добавить '@'.
    const std::string in = "if(x){ y:=1; }";
    const std::string exp = "@if (x) {\n    y := 1;\n}\n";
    EXPECT_EQ(fmt(in, opts), exp);
}

TEST(FormatterTest, KeywordSigilNone) {
    // None — оставить '@' как в исходнике (@if остаётся @if).
    FormatOptions opts;
    opts.keywords = {"if"};
    opts.keyword_sigil = KeywordSigil::None;
    const std::string in = "@if(x){ y:=1; }";
    const std::string exp = "@if (x) {\n    y := 1;\n}\n";
    EXPECT_EQ(fmt(in, opts), exp);
}

TEST(FormatterTest, KeywordSigilRemoveBareUntouched) {
    // Remove для bare (без '@') — не добавляет и не убирает ничего.
    FormatOptions opts;
    opts.keywords = {"if"};
    opts.keyword_sigil = KeywordSigil::Remove;
    const std::string in = "if(x){ y:=1; }";
    const std::string exp = "if (x) {\n    y := 1;\n}\n";
    EXPECT_EQ(fmt(in, opts), exp);
}
