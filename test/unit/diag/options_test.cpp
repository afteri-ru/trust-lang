...
TEST(Options, SeverityIgnored) {
    ...
    Options opts(diag);
    opts.add(semantic::DiagId::UnusedVariable);
    opts.set(semantic::DiagId::UnusedVariable, Severity::Ignore);
    EXPECT_EQ(opts.get(semantic::DiagId::UnusedVariable), Severity::Ignore);
    EXPECT_EQ(opts.getByName("unused-variable"), Severity::Ignore);
}
...
TEST(Options, ParseArgv_WallOnlyAffectsWallGroup) {
    ...
    opts.parse_argv(argv);
    EXPECT_EQ(opts.get(semantic::DiagId::UnusedVariable), Severity::Warning);
    EXPECT_EQ(opts.get(semantic::DiagId::WidenAny), Severity::Ignore);
}
...
TEST(Options, ParseArgv_WunusedGroup) {
    ...
    opts.parse_argv(argv);
    EXPECT_EQ(opts.get(semantic::DiagId::UnusedVariable), Severity::Warning);
    EXPECT_EQ(opts.get(diag::DiagId::Deprecated), Severity::Ignore);
}
...
TEST(Options, ParseArgvWnoSeverityDisables) {
    // `-Wno-<name>` выключает severity-диагностику (Severity::Ignore), стиль clang/gcc.
    ...
    opts.parse_argv(argv);
    EXPECT_EQ(opts.get(semantic::DiagId::UnusedVariable), Severity::Ignore);
    EXPECT_EQ(opts.get(semantic::DiagId::Embed), Severity::Ignore);
    EXPECT_EQ(opts.get(semantic::DiagId::NoSigil), Severity::Ignore);
    EXPECT_EQ(opts.get(semantic::DiagId::Solver), Severity::Ignore);
}
...
TEST(Options, HelpTopicDetection) {
    DiagnosticEngine diag;
    Options opts(diag);
    // По умолчанию справка не запрошена.
    EXPECT_FALSE(opts.helpRequested());
    EXPECT_EQ(opts.helpTopic(), Options::HelpTopic::None);

    // -Whelp — справка по диагностикам.
    char* argv1[] = {const_cast<char*>("-Whelp")};
    opts.parse_argv(argv1);
    EXPECT_TRUE(opts.helpRequested());
    EXPECT_EQ(opts.helpTopic(), Options::HelpTopic::Diagnostics);
}

TEST(Options, HelpTopicDsl) {
    DiagnosticEngine diag;
    Options opts(diag);
    // -Whelp-dsl — справка по макросам загруженного DSL.
    char* argv[] = {const_cast<char*>("-Whelp-dsl")};
    opts.parse_argv(argv);
    EXPECT_TRUE(opts.helpRequested());
    EXPECT_EQ(opts.helpTopic(), Options::HelpTopic::DslMacros);
}

TEST(Options, HelpTopicPredefMacros) {
    DiagnosticEngine diag;
    Options opts(diag);
    // -Whelp-predef-macros — справка по предопределённым макросам @__...__.
    char* argv[] = {const_cast<char*>("-Whelp-predef-macros")};
    opts.parse_argv(argv);
    EXPECT_TRUE(opts.helpRequested());
    EXPECT_EQ(opts.helpTopic(), Options::HelpTopic::PredefMacros);
}
...
TEST(Options, FlagCommentsNonW) {
    DiagnosticEngine diag;
    Options opts(diag);
    // Comments — поведенческий флаг (НЕ управляется через -W): регистрируется через add_flag_nonw.
    opts.add_flag_nonw(transpiler::FlagKind::Comments);
    EXPECT_FALSE(opts.is_enabled(transpiler::FlagKind::Comments));
    EXPECT_TRUE(opts.setEnabledByName("comments", true));
    EXPECT_TRUE(opts.is_enabled(transpiler::FlagKind::Comments));
    // -Wcomments для non-W флага — неизвестная опция (бросает invalid_argument).
    {
        char a[] = "-Wcomments";
        char* argv[] = {a};
