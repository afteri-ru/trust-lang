// test/unit/driver/analysis_options_test.cpp
// Юнит-тесты единой точки применения опций анализа (driver/analysis_options.hpp):
...
TEST(AnalysisOptions, SolverModeAndSigilOrderIndependent) {
    ...
    // -Wsigil=ignore: NoSigil выключен (Severity::Ignore).
    EXPECT_EQ(ctx.opts().get(semantic::DiagId::NoSigil), Severity::Ignore);
}
...
// Контроль знакового целочисленного переполнения: поведенческий флаг транспилятора
// (-foverflow-check/-fno-overflow-check), default = ON (безопасность по умолчанию).
TEST(AnalysisOptions, OverflowCheckFlag) {
    Context ctx;
    EXPECT_TRUE(ctx.opts().isEnabledByName("overflow-check"));
    const std::vector<std::string> off = {"-fno-overflow-check"};
    EXPECT_EQ(applyAnalysisArgs(ctx.opts(), off), "");
    EXPECT_FALSE(ctx.opts().isEnabledByName("overflow-check"));
    const std::vector<std::string> on = {"-foverflow-check"};
    EXPECT_EQ(applyAnalysisArgs(ctx.opts(), on), "");
    EXPECT_TRUE(ctx.opts().isEnabledByName("overflow-check"));
}
...
TEST(AnalysisOptions, EmptyArgsNoEffect) {
    ...
    EXPECT_EQ(ctx.opts().get(semantic::DiagId::Solver), Severity::Warning); // -Wsolver остаётся default
}
