// test/unit/driver/parse_test.cpp - unit-тесты arity-aware парсера драйвера (Pipeline::parseArgs).
...
// Использует фиксированный каталог в _build (не уникальные временные имена).
std::string create_temp_trust_file() {
    static const std::string dir = trust::test::makeTestDataDir("pipeline_test").string();
