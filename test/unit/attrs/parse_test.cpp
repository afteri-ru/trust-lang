// test/unit/attrs/parse_test.cpp - тесты разбора атрибутов (@[...]) и битмаски AttrId.
...
TEST_F(AttrParserTest, PinAndLifetimeRegistered) {
    // pin: без параметров; обрабатывается анализатором (pin-механика - отдельная ветка).
    auto pin = m_ctx.attrs().lookup(attr::Pin);
    ASSERT_TRUE(pin.has_value());
    EXPECT_TRUE(detail::is_builtin(*pin));
    EXPECT_TRUE(detail::is_analyzer_supported(*pin));
    EXPECT_FALSE(detail::is_codegen_supported(*pin));
    EXPECT_TRUE(detail::is_handled(*pin));

    // lifetime: 1..2 аргумента (area[, name]); обрабатывается анализатором.
    auto lt = m_ctx.attrs().lookup(attr::Lifetime);
    ASSERT_TRUE(lt.has_value());
    EXPECT_TRUE(detail::is_builtin(*lt));
    EXPECT_TRUE(detail::is_analyzer_supported(*lt));
    EXPECT_TRUE(detail::is_handled(*lt));
    const Attr& attr = m_ctx.attrs().get(*lt);
    EXPECT_EQ(attr.m_default_params.size(), 2u);
    EXPECT_TRUE(attr.matches_params({"self"}));
    EXPECT_TRUE(attr.matches_params({"param", "x"}));
    EXPECT_FALSE(attr.matches_params({}));
    EXPECT_FALSE(attr.matches_params({"a", "b", "c"}));
}
