...
TEST(SaveAndReadTest, SaveOutputThenReadFromDisk) {
    // Используем фиксированный каталог в TEST_DATA_DIR (_build/test_data/)
    fs::path dataDir = trust::test::makeTestDataDir("save_read_test");
...
TEST(SaveAndReadTest, ChecksumMismatchDetected) {
    ...
    fs::path dataDir = trust::test::makeTestDataDir("checksum_test");
    std::string outputPath = (dataDir / "output.cpp").generic_string();
    std::string inputPath = (dataDir / "test.src").generic_string();
...
TEST(SaveAndReadTest, DiskReadFailure) {
    ...
    fs::path dataDir = trust::test::makeTestDataDir("disk_read_fail_test");
    std::string outputPath = (dataDir / "output.cpp").generic_string();
    std::string inputPath = (dataDir / "test.src").generic_string();
