...
TEST(MappingExtTest, StressTest_500Mappings) {
    ...
    // Сохраняем на диск
    std::string filePath = (trust::test::makeTestDataDir("mapping_extended") / "stress_500.msgpack").string();
...
TEST(MappingExtTest, StressTest_1000RangeMappings) {
    ...
    // Сохраняем на диск
    std::string filePath = (trust::test::makeTestDataDir("mapping_extended") / "stress_1000.msgpack").string();
    writeMsgpackToFile(filePath, packed);
