...
// -- Встроенный контекст-маркер @__CHECK_AREA__(...) из исходника → AST-узел CheckAreaStmt -- //
// Маркер захватывается в GetNextToken (PragmaEvaluator::evalCheckArea), превращается в единый
// терм-маркер MACRO_CONTEXT, а term_to_ast строит из него лист CheckAreaStmt с полями
// area/behavior (атрибуты-требования - в attrs() узла). Семантика удаляет маркер (кода не даёт).
TEST_F(ParserTest, CheckAreaSourceParsesToMarker) {
    ASSERT_TRUE(Parse("@f() := { @__CHECK_AREA__(function, error); };"));
    std::vector<AstNodePtr> nodes = TermToAstConverter::termToAst(ast, m_ctx);

    CheckAreaStmt* found = nullptr;
    for (auto& n : nodes) {
        if (!n) {
            continue;
        }
        if (auto* fn = dynamic_cast<FuncDecl*>(n.get())) {
            if (fn->m_body) {
                for (auto& b : *fn->m_body) {
                    if (b && b->kind() == ParserToken::Kind::CheckAreaStmt) {
                        found = dynamic_cast<CheckAreaStmt*>(b.get());
                    }
                }
            }
        }
    }
    ASSERT_NE(found, nullptr) << "@__CHECK_AREA__ must reach the AST as a CheckAreaStmt";
    ASSERT_TRUE(found->area.has_value());
    EXPECT_EQ(*found->area, AreaKind::Function);
    ASSERT_TRUE(found->behavior.has_value());
    EXPECT_EQ(*found->behavior, Severity::Error);
}

TEST_F(ParserTest, CheckAreaParsesDefaultBehaviorAsNullopt) {
    // Без поведения (или с 'default') поле behavior = nullopt (→ -Wcheck-area), а не Error/Warning.
    ASSERT_TRUE(Parse("@f() := { @__CHECK_AREA__(\"function\", default); };"));
    std::vector<AstNodePtr> nodes = TermToAstConverter::termToAst(ast, m_ctx);
    CheckAreaStmt* found = nullptr;
    for (auto& n : nodes) {
        if (auto* fn = dynamic_cast<FuncDecl*>(n.get())) {
    ...
    ASSERT_TRUE(found->area.has_value());
    EXPECT_EQ(*found->area, AreaKind::Function);
    EXPECT_FALSE(found->behavior.has_value());
}

TEST_F(ParserTest, CheckAreaBadBehaviorIsStrictParseError) {
    // Не-поведение во 2-й позиции (строгий разбор) - ошибка; маркер не строится.
    Parse("@f() := { @__CHECK_AREA__(function, bogus); };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}

TEST_F(ParserTest, CheckAreaUnknownAreaIsError) {
    Parse("@f() := { @__CHECK_AREA__(no_such_area, error); };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}
...
TEST_F(ParserTest, AssignSimpleArg) {
    // RHS у `::=` - ТОЛЬКО тип; вызов `term2(arg)` в RHS типа - ОШИБКА.
    Parse("\t term    ::=    term2( arg  ) ;   \n");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}
...
TEST_F(ParserTest, AssignStringMultiline) {
    /* Строка с переносом '\' + перевод строки склеивается в однострочную строку:
     * маркер '\'+перевод строки удаляется, содержимое следующей строки продолжает
     * литерал. Esc-последовательности сохраняются как есть. */
    ASSERT_TRUE(Parse("term  :=  'ab\\\ncd\\t\\s' ; \n"));
    ASSERT_EQ("term := 'abcd\\t\\s';", ast->toString());
}
...
TEST_F(ParserTest, Func3) {
    // RHS у `::=` - ТОЛЬКО тип; вызов функции в RHS типа - ОШИБКА (не «общее выражение»).
    Parse("$res:Int8 ::= func_arg(100, 100);");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}
