...
// Многострочный макрос (раскрытое тело содержит ';'), использующий аргумент @$name
// несколько раз: подстановка аргумента выполняется КЛОНОМ (регрессия - общий токен при
// повторном использовании аргумента ломал разбор/типы). Раскрытие не должно бросать
// EXPECT(b<=e)/FAULT коллизии и должно давать корректный текст.
TEST_F(MacroTest, MultilineMacro_CloneAndNoRangeCrash) {
    MacroPtr macro = std::make_shared<Macro>(m_ctx);
    const std::string src = "@@ two( $name ) @@\n"
                            "@$name := 1;\n"
                            "@$name = @$name + 1;\n"
                            "@@@@;\n"
                            "@two(v);\n";
    // Не должно бросать: ни EXPECT(b<=e) при комбинации диапазонов, ни FAULT коллизии.
    ASSERT_NO_THROW(Parse(src, macro));

    // Раскрытие @two(v) даёт `v := 1; v = v + 1;` (аргумент подставлен клоном).
    const std::string out = LexOut();
    EXPECT_NE(out.find("v := 1"), std::string::npos) << out;
    EXPECT_NE(out.find("v = v + 1"), std::string::npos) << out;
}
