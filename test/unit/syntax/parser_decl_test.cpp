...
TEST_F(ParserTest, NativeClassForwardDecl) {
    // Forward-объявление нативного класса / шаблон-класса через `::=`.
    EXPECT_TRUE(Parse("@include(\"string\") String ::= %std::string { };"));
    EXPECT_TRUE(Parse("String ::= %std::string { %size(): UInt64 := ...; @::st(): Void := ...; };"));
    // Явная реализация шаблон-класса `<T1,T2> Pair ::= %std::pair<T1,T2> { ... };`.
    EXPECT_TRUE(Parse("@include(\"utility\") <T1,T2> Pair ::= %std::pair<T1,T2> { %first(): T1 := ...; };"));
    // Обобщённая форма (сахар) парсится; диагностика «not implemented» - в семантике.
    EXPECT_TRUE(Parse("<T1,T2> Pair ::= <T1,T2> %std::pair { };"));
    // RHS - ЛЮБОЕ имя (native - с ведущим '%'; обычное имя тоже допустимо).
    EXPECT_TRUE(Parse("Cls ::= std::shared_ptr { };"));
}
...
TEST_F(ParserTest, Class) {
    // Определение класса через `::=`: базы БЕЗ скобок (типы-ссылки), допускается перечисление
    // для множественного наследования и шаблонные базы `:Class<:T>`. Тело - поля/методы через `:=`.
    EXPECT_TRUE(Parse(":Name ::= :Class{ x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "simple class def";
    EXPECT_TRUE(Parse(":Name ::= :Class1, :Class<:T>{ x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "multi-inheritance with template base";
    EXPECT_TRUE(Parse(":Name ::= :Class<:T>{ x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "template base";
    EXPECT_TRUE(Parse(":Name ::= :Class<:Int32>{ x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "template base with concrete arg";
    EXPECT_TRUE(Parse(":Name ::= :ns::Class{ x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "namespaced base";
    EXPECT_TRUE(Parse(":Name ::= :_{ filed1 = 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "anonymous base";

    // Методы/поля/конструктор/деструктор в теле класса - ТОЛЬКО через `:=`.
    EXPECT_TRUE(Parse(":Name ::= :Class{ m() := {}; x := 1; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "method + field";
    EXPECT_TRUE(Parse(":Name ::= :Class{ func1() := {}; func2() := {}; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "two methods via :=";
    EXPECT_TRUE(Parse(":Name ::= :Class{ Name() := Class1(), :Class<:T>(100){ }; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "constructor with base calls (incl. template)";
    EXPECT_TRUE(Parse(":Name ::= :Class{ ~Name() := { }; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "destructor";
    EXPECT_TRUE(Parse(":Name ::= :Class{ m() := {}; x := 1; ~Name() := { }; };"));
    EXPECT_EQ(0, m_ctx.diag().errorCount()) << "method + field + destructor";

    // Конструктор вне `::=` (метод с телом-классом через `:=`).
    EXPECT_TRUE(Parse("Name() := :Class(), Class(){};"));
    EXPECT_TRUE(Parse("::Name() := ::Func(){ func1() := {}; func2() := {};};"));

    // Негативные случаи (ошибки синтаксиса, Parse возвращает не-null, проверяем errorCount).
    Parse(":Name ::= :Class(){ x := 1; };");
    EXPECT_GT(m_ctx.diag().errorCount(), 0) << "bases must not have parentheses";
    Parse(":Point ::= { x := 0; };");
    EXPECT_GT(m_ctx.diag().errorCount(), 0) << "class must have at least one base";
    Parse(":Name ::= :Class{ x := 1 };");
    EXPECT_GT(m_ctx.diag().errorCount(), 0) << "class body must end with ';'";
    Parse(":Name ::= :Class{ func() ::= {}; };");
    EXPECT_GT(m_ctx.diag().errorCount(), 0) << "methods are created only via ':='";
}
...
TEST_F(ParserTest, FuncAssignOpsCreateNew) {
    // `::=` - ТОЛЬКО тип; `func(...) ::= { }` (функция с телом-блоком через ::=) - ОШИБКА
    // (функция определяется через `:=`).
    Parse("func(arg) ::= { };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);

    Parse("func(arg1, arg2=2) ::= { };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);

    Parse("func(arg:Int32=1, arg2) ::= { };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}
...
TEST_F(ParserTest, FuncFullCombinations) {
    ...
    // `::=` - ТОЛЬКО тип; функция с телом-блоком через `::=` - ОШИБКА (функция - через `:=`).
    Parse("func(a:Int32=val, b, c=2) ::= { };");
    ASSERT_GT(m_ctx.diag().errorCount(), 0);
}
