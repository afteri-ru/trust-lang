...
std::pair<std::string, std::string> parse_attr(const std::string& token) {
    ...
    for (size_t i = 1; i < token.size(); ++i) {
        if (token[i] == '=') {
            bool prev_is_key_char = (token[i-1] >= 'a' && token[i-1] <= 'z') ||
...
static std::unique_ptr<ParsedNode> parse_nodes(std::vector<std::string>& lines, const std::string& text, SourceIdx src, size_t& pos, int parent_indent, std::string& error_msg) {
    ...
    if (indent <= parent_indent && pos > 0) return nullptr;
    ...
    std::string token;
    while (iss >> token) node->attrs.push_back(parse_attr(token));
    pos++;
    ...
    while (pos < lines.size()) {
        ...
        if (count_indent(lines[pos]) <= indent) break;
...
parse_ast_format(const std::string& text, Context& ctx) {
    ...
    while (pos < lines.size()) {
        ...
        if (node) roots.push_back(std::move(node));
        else {
            if (!error_msg.empty())
...
static std::optional<BlockItem> build_block_item(const ParsedNode* n, Context& ctx) {
    ...
    if (cat == TokenCategory::Decl) {
        ...
        if (n->kind == ParserToken::Kind::ThenBlock ||
            n->kind == ParserToken::Kind::ElseBlock ||
            n->kind == ParserToken::Kind::MatchingElseBlock ||
            n->kind == ParserToken::Kind::MatchingCase ||
            n->kind == ParserToken::Kind::WhileElseBlock) {
            return std::nullopt;
        }
...
static std::unique_ptr<Expr> build_assignment_target(const std::string& target_str, Context& ctx) {
    ...
    if (bracket_pos != std::string::npos) {
        ...
        try { idx = std::make_unique<IntLiteral>(std::stoi(idx_str)); }
        catch (...) { idx = std::make_unique<VarRef>(idx_str); }
        auto arr_access = std::make_unique<ArrayAccess>(std::move(var_ref), std::move(idx));
        if (!field.empty()) return std::make_unique<MemberAccess>(std::move(arr_access), field);
        return arr_access;
    }
    auto dot_pos = target_str.find('.');
    if (dot_pos != std::string::npos) {
...
static std::expected<std::unique_ptr<Expr>, std::string> build_expr(const ParsedNode* n, Context& ctx) {
    if (!n) return std::unexpected("null expression node");
    auto tk = n->kind;

    if (tk == ParserToken::Kind::IntLiteral) {
        if (!has_attr(n, "value")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'value'");
        std::string v = get_attr(n, "value");
        try { auto result = std::make_unique<IntLiteral>(std::stoi(v)); result->loc = n->loc; return result; }
        catch (const std::invalid_argument&) { return std::unexpected("IntLiteral: invalid integer '" + v + "'"); }
        catch (const std::out_of_range&) { return std::unexpected("IntLiteral: out of range '" + v + "'"); }
    }
    ...
    if (tk == ParserToken::Kind::VarRef) {
        if (!has_attr(n, "name")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'name'");
        auto result = std::make_unique<VarRef>(get_attr(n, "name"));
        result->loc = n->loc;
        return result;
    }
    if (tk == ParserToken::Kind::BinaryOp) {
        if (!has_attr(n, "op")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'op'");
        if (n->children.size() < 2) return std::unexpected(AstTypeTraits::to_string(tk) + " requires 2 children");
        auto op = BinOpUtils::from_string(get_attr(n, "op"));
    ...
    if (tk == ParserToken::Kind::EnumLiteral) {
        if (!has_attr(n, "enum")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'enum'");
        if (!has_attr(n, "member")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'member'");
        auto result = std::make_unique<EnumLiteral>(get_attr(n, "enum"), get_attr(n, "member"));
        result->loc = n->loc;
        return result;
    }
    if (tk == ParserToken::Kind::MemberAccess) {
        if (!has_attr(n, "field")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'field'");
        if (n->children.empty()) return std::unexpected(AstTypeTraits::to_string(tk) + " requires object child");
    ...
    if (tk == ParserToken::Kind::ArrayInit) {
        ...
        if (has_attr(n, "type")) {
            ...
            if (!ti.has_value()) return std::unexpected(ti.error());
            elem_type = ti->kind;
        }
        for (auto& c : n->children) {
    ...
    if (tk == ParserToken::Kind::CastExpr) {
        if (!has_attr(n, "type")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'type'");
        if (n->children.empty()) return std::unexpected(AstTypeTraits::to_string(tk) + " requires an expression child");
    ...
    if (tk == ParserToken::Kind::RefMakeExpr) {
        if (n->children.empty()) return std::unexpected(AstTypeTraits::to_string(tk) + " requires a child");
...
static std::unique_ptr<CatchBlock> build_catch_block(const ParsedNode* n, Context& ctx) {
    if (!n || n->kind != ParserToken::Kind::CatchBlock) return nullptr;
    if (!has_attr(n, "type") || !has_attr(n, "name")) return nullptr;
    ...
    for (auto& c : n->children) {
        auto item = build_block_item(c.get(), ctx);
        if (item.has_value()) body.push_back(std::move(*item));
    }
...
static std::expected<std::unique_ptr<Stmt>, std::string> build_stmt(const ParsedNode* n, Context& ctx) {
    if (!n) return std::unexpected("null statement node");
    auto tk = n->kind;

    if (tk == ParserToken::Kind::AssignmentStmt) {
        if (!has_attr(n, "target")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'target'");
    ...
    if (tk == ParserToken::Kind::IfStmt) {
        ...
        if (i < n->children.size() && n->children[i]->is_expr()) {
            ...
            if (!c.has_value()) return std::unexpected(c.error());
            cond = std::move(*c);
            i++;
        }
        BlockBody then_body;
        while (i < n->children.size() && n->children[i]->kind != ParserToken::Kind::ElseBlock) {
            if (n->children[i]->kind == ParserToken::Kind::ThenBlock) {
                for (auto& c : n->children[i]->children) {
                    auto item = build_block_item(c.get(), ctx);
                    if (item.has_value()) then_body.push_back(std::move(*item));
                }
            } else {
                auto item = build_block_item(n->children[i].get(), ctx);
                if (item.has_value()) then_body.push_back(std::move(*item));
        ...
        if (i < n->children.size() && n->children[i]->kind == ParserToken::Kind::ElseBlock) {
            ...
            if (else_blk->children.size() == 1 && else_blk->children[0]->kind == ParserToken::Kind::IfStmt) {
                ...
                if (!elif_node) return elif_node;
                else_if = std::unique_ptr<IfStmt>(static_cast<IfStmt*>(elif_node->release()));
            } else {
                BlockBody else_nodes;
                for (auto& c : else_blk->children) {
                    auto item = build_block_item(c.get(), ctx);
                    if (item.has_value()) else_nodes.push_back(std::move(*item));
                }
                if (!else_nodes.empty()) else_block = std::make_unique<BlockStmt>(std::move(else_nodes));
            }
        }
    ...
    if (tk == ParserToken::Kind::WhileStmt) {
        ...
        for (; i < n->children.size(); ++i) {
            if (n->children[i]->kind == ParserToken::Kind::WhileElseBlock) {
                for (auto& c : n->children[i]->children) {
                    auto item = build_block_item(c.get(), ctx);
                    if (item.has_value()) else_body.push_back(std::move(*item));
                }
            } else {
                auto item = build_block_item(n->children[i].get(), ctx);
                if (item.has_value()) body.push_back(std::move(*item));
    ...
    if (tk == ParserToken::Kind::TryCatchStmt) {
        ...
        for (; i < n->children.size() && n->children[i]->kind != ParserToken::Kind::CatchBlock; ++i) {
            auto item = build_block_item(n->children[i].get(), ctx);
            if (item.has_value()) try_body.push_back(std::move(*item));
        }
        ...
        if (i < n->children.size() && n->children[i]->kind == ParserToken::Kind::CatchBlock) {
            ...
            if (!catch_blk) return std::unexpected("failed to build catch block");
        }
    ...
    if (tk == ParserToken::Kind::VarDecl || tk == ParserToken::Kind::FuncDecl ||
        tk == ParserToken::Kind::ParamDecl || tk == ParserToken::Kind::EnumDecl ||
        tk == ParserToken::Kind::EnumMember || tk == ParserToken::Kind::StructDecl ||
        tk == ParserToken::Kind::StructField) {
...
static std::expected<std::unique_ptr<Decl>, std::string> build_decl(const ParsedNode* n, Context& ctx) {
    if (!n) return std::unexpected("null declaration node");
    auto tk = n->kind;
    ...
    if (tk == ParserToken::Kind::VarDecl) {
        ...
        if (!n->children.empty()) {
            ...
            if (!i.has_value()) return std::unexpected(i.error());
            init = std::move(*i);
        }
        if (has_attr(n, "type")) {
    ...
    if (tk == ParserToken::Kind::FuncDecl) {
        if (!has_attr(n, "name")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'name'");
        if (!has_attr(n, "ret")) return std::unexpected(AstTypeTraits::to_string(tk) + ": missing required attribute 'ret'");
        std::string name = get_attr(n, "name");
        auto ret_ti = parse_type_internal(get_attr(n, "ret"), "FuncDecl");
        if (!ret_ti.has_value()) return std::unexpected(ret_ti.error());
    ...
    if (tk == ParserToken::Kind::EnumDecl) {
        ...
        for (auto& c : n->children) {
            if (c->kind == ParserToken::Kind::EnumMember) {
                if (!has_attr(c.get(), "name")) return std::unexpected(AstTypeTraits::to_string(c->kind) + ": missing 'name'");
    ...
    if (tk == ParserToken::Kind::StructDecl) {
        ...
        for (auto& c : n->children) {
            if (c->kind == ParserToken::Kind::StructField) {
                if (!has_attr(c.get(), "name")) return std::unexpected(AstTypeTraits::to_string(c->kind) + ": missing 'name'");
                if (!has_attr(c.get(), "type")) return std::unexpected(AstTypeTraits::to_string(c->kind) + ": missing 'type'");
                ...
                for (auto& cc : c->children) {
                    if (cc->is_expr()) {
                        ...
                        if (!vd.has_value()) return std::unexpected(vd.error());
                        if (auto* v = dynamic_cast<VarDecl*>(vd->get()))
                ...
                if (!method.has_value()) return std::unexpected(method.error());
...
std::unique_ptr<Program> build_ast_from_roots(const std::vector<ParsedNode*>& roots, Context& ctx) {
    ...
    for (auto* node : roots) {
        ...
        if (cat == TokenCategory::Decl || cat == TokenCategory::Root) {
            ...
            if (decl) items.push_back(std::move(decl));
        } else {
            auto s = build_statement(node, ctx);
            if (s) stmt_body.push_back(std::move(s));
        }
    }
    if (!stmt_body.empty()) {
        std::vector<std::unique_ptr<ParamDecl>> no_params;
        items.push_back(std::make_unique<FuncDecl>("main", TypeKind::Int,
                                        std::move(no_params),
                                        std::make_unique<BlockStmt>(std::move(stmt_body))));
    }
