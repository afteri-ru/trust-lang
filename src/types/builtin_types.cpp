...
void TypeRegistry::registerBuiltinTypes() {
    ...
    // Interruptions (runtime классы перехвата прерываний): IntAny - база, IntPlus - возврат,
    // IntMinus - ошибка. Транслируются в trust::IntAny/IntPlus/IntMinus (заголовок interrupt.hpp).
    registerBuiltinType(type::IntAny, Group::kExceptions, 0, "trust::IntAny", {"@trust/interrupt.hpp"});
    registerBuiltinType(type::IntPlus, Group::kExceptions, 0, "trust::IntPlus", {"@trust/interrupt.hpp"});
    registerBuiltinType(type::IntMinus, Group::kExceptions, 0, "trust::IntMinus", {"@trust/interrupt.hpp"});
    // Базовый C++-класс исключений: catch(:StdException) → `catch (std::exception&)`.
    // Обобщённый catch - ловит любое std::exception-производное (ошибки конверсии и т.п.),
    // а не только прерывания trust. Инклуд <stdexcept> обязателен (std::exception).
    registerBuiltinType(type::StdException, Group::kExceptions, 0, "std::exception", {"#include <stdexcept>"});
...
TypeId TypeRegistry::registerBuiltinType(std::string_view name, Group group, uint8_t data, std::string_view cpp_name, std::vector<std::string> preprocIncludes,
                                         bool markAttrs) {
    ...
    if (markAttrs) {
        kind = setAttrsFlag(kind); // признак «тип несёт атрибуты» (напр. политики sync)
    }
