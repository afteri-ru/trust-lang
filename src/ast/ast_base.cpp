...
: ArgNode(std::move(term)) {
    if (ctx && m_term) {
        // Иммутабельность ('^' в имени параметра, напр. `argv^`) → attr::ReadOnly.
        // Базовый терм-конструктор нормализует текст (срезает '^'), но признак не ставит;
        // здесь применяем его единым хелпером (как convertAttrsToNode/IdentName), чтобы
        // кодогенерация могла эмитить `const` для read-only параметров.
        applyReadonlyFromCaret(*this, declNameFromTerm(m_term), &ctx->attrs());
