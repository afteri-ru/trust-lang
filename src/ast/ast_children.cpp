...
void AstNodeBase::collectChildren(std::vector<AstNodePtr*>& out) {
    ...
    // -- ClassDecl: члены интерфейса (методы/поля/конструкторы/статич-члены) --
    case ParserToken::Kind::ClassDecl: {
        auto& n = static_cast<ClassDecl&>(*this);
        for (auto& c : n.m_body) {
            out.push_back(&c);
        }
        break;
    ...
    case ParserToken::Kind::ContinueStmt: {
        ...
        // Синтезированная временная объявляется ДО m_value (семантика ставит ссылку на неё).
        if (j.m_tempDecl) {
            out.push_back(&j.m_tempDecl);
        }
        if (j.m_value) {
    ...
    case ParserToken::Kind::DoWhileStmt: {
        ...
        if (m_kind == ParserToken::Kind::WhileStmt) {
            if (auto* w = static_cast<WhileStmt*>(this); w->m_elseFlag) {
                out.push_back(&w->m_elseFlag);
            }
        }
    ...
    // -- LastResultCapture: temp (временная) + source (источник) + sink (потребитель) --
    case ParserToken::Kind::LastResultCapture: {
        auto& c = static_cast<LastResultCapture&>(*this);
        if (c.m_temp) {
            out.push_back(&c.m_temp);
        }
        if (c.m_source) {
            out.push_back(&c.m_source);
        }
        if (c.m_sink) {
            out.push_back(&c.m_sink);
        }
        break;
    }
    ...
    case ParserToken::Kind::MatchingStmt: {
        ...
        // Синтезированная временная объявляется ДО m_value (lowering ставит ссылку на неё).
        if (m.m_tempDecl) {
            out.push_back(&m.m_tempDecl);
        }
        if (m.m_value) {
    ...
    // -- TryCatchStmt: тело try + ветки catch (m_catches) + else (m_else) --
    case ParserToken::Kind::TryCatchStmt: {
        auto& tc = static_cast<TryCatchStmt&>(*this);
        for (auto& c : tc.m_body) {
            out.push_back(&c);
        }
        for (auto& cb : tc.m_catches) {
            out.push_back(&cb);
        }
        if (tc.m_else) {
            out.push_back(&tc.m_else);
        }
        break;
    }
    // -- WithStmt: биндинги + тело + else -- (для общих обходов; семантика и транспилятор
    // обрабатывают детей явно в своём кейсе/visit, поэтому повторного обхода не будет).
    case ParserToken::Kind::WithStmt: {
        auto& w = static_cast<WithStmt&>(*this);
        ...
        if (w.m_body) {
            out.push_back(&w.m_body);
        }
        if (w.m_else) {
            out.push_back(&w.m_else);
        }
        break;
    }
    default:
