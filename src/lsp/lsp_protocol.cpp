...
LspOptions parseLspOptions(int argc, const char* argv[]) {
    ...
    auto apply = [&](int id, const std::string& v) -> bool {
        ...
        case LspOptId::HtmlFull:
            // `--html-full` - самостоятельная форма `--html` (полная страница вместо фрагмента):
            // включает HTML-режим, даже если `--html` не передавали (иначе --html-full без
            // --html молча уходил бы в interactive-режим с пустым выводом).
            opts.htmlFull = true;
            opts.mode = LspMode::Html;
            break;
