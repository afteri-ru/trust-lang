...
std::size_t ModuleLoader::parseSourceModule(std::string_view moduleId, MapperFile src, MapperRange range) {
    ...
    try {
        ...
        auto body = std::make_shared<std::vector<AstNodePtr>>(termToAst(term, m_ctx));
        registry().setPreprocessed(idx, body);
        registry().setCacheApi(idx, body);
    } catch (...) {
...
const std::vector<AstNodePtr>& ModuleLoader::preprocessed(std::size_t index) const {
    return registry().preprocessed(index);
}
