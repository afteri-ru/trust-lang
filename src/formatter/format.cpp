...
class Emitter {
  ...
  private:
    ...
    bool isKeywordText(const std::string& t) const {
        ...
        // встроенного набора). Распознавание не зависит от ведущего '@' (в обеих сторонах):
        // и '@if'/'@return', и 'if'/'return' трактуются как ключевое слово (важно для пробела
        // перед '(' и '{', а также согласовано с опцией KeywordSigil).
        const std::string_view tv = trust::utils::strip_macro_sigil(t);
    ...
    // Текст токена с применением опции обработки '@'-сигила ключевых слов (KeywordSigil):
    //   Remove — убрать ведущий '@' (@if → if); Add — добавить (if → @if); None — как есть.
    // Применяется только к токенам, распознанным как ключевое слово (isKeywordLike); прочие —
    // без изменений (raw/embed/определения макросов не трогаем).
    std::string keywordText(const Token& tok) const {
        if (!isKeywordLike(tok.text)) {
            return tok.text;
        }
        const std::string_view bare = trust::utils::strip_macro_sigil(tok.text);
        switch (opts_.keyword_sigil) {
        case KeywordSigil::Remove:
            return std::string(bare);
        case KeywordSigil::Add:
            return (!tok.text.empty() && tok.text[0] == '@') ? tok.text : "@" + std::string(bare);
        case KeywordSigil::None:
        default:
            return tok.text;
        }
    }
    ...
    bool computeSpace(int prev, int i) {
        ...
        // Нет пробела после ':' (сигил типа ':Int32'). Внутри raw-контракта `@{ expr @}` kind НЕ
        // указывается (вынесен перед маркером), поэтому ':' в выражении форматируется единообразно.
