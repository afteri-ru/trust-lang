...
FormatConfig parseConfig(std::string_view text) {
    ...
    while (pos < text.size()) {
        ...
        } else if (pl.key == "KeywordSigil") {
            if (!keywordSigilFromString(pl.value, cfg.opts.keyword_sigil)) {
                cfg.ok = false;
                cfg.error = "KeywordSigil: expected remove|add|none, got '" + pl.value + "'";
                return cfg;
            }
        } else {
            cfg.ok = false;
...
std::string dumpConfig(const FormatOptions& opts) {
    ...
    out += "\n";
    out += "# Handle the '@' sigil on all keywords: remove | add | none (default: remove).\n";
    out += "KeywordSigil: ";
    out += keywordSigilToString(opts.keyword_sigil);
    out += "\n";
