...
void ExprEmitter::visit_IntLiteral(const Literal& n) {
    // Разделители разрядов '_' срезаются перед эмитом: целевой C++ (clang в этой toolchain)
    // не принимает `10_000` как разделитель разрядов, а значение литерала от формата не зависит.
    const std::string cleaned = stripDigitSeparators(n.text());
    // Сверхразрядный целочисленный литерал (вне Int64) типизирован как BigInteger: эмитим
    // `trust::BigInteger("...")` - десятичный строковый конструктор. Постфиксная аннотация
    // `N :Rational` → `trust::Rational("N", "1")` (числитель/знаменатель; работает и для
    // сверхразрядных значений). Инклуды записываются через emitTypeName (единая точка сбора).
    const TypeRegistry& reg = m_ectx.m_ctx.types();
    ...
    const TypeId tid = (n.typeId != INVALID_TYPE_ID) ? reg.getCanonicalTypeId(n.typeId) : INVALID_TYPE_ID;
    if (tid == bi) {
        m_driver.m_type.emitTypeName(bi, "BigInteger");
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, std::format("trust::BigInteger(\"{}\")", cleaned));
        return;
    }
    if (tid == rat) {
        m_driver.m_type.emitTypeName(rat, "Rational");
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, std::format("trust::Rational(\"{}\", \"1\")", cleaned));
        return;
    }
    m_ectx.m_ctx.source().output_append(m_ectx.m_out, cleaned);
}
...
// `/** … */` и `///` выводится как есть. Подавление (флаг -fno-comments) - на уровне обхода.
void ExprEmitter::visit_Document(const AstNodeAttr& n) {
    if (m_ectx.m_exprDepth > 0) {
        return;
    }
    // Комментарий всегда с новой строки: если текущая позиция НЕ в начале строки (напр. сразу после
    // '}' функции — комментарий раскрытого макроса `/// expanded macro ...`), добавляем перевод
    // ПЕРЕД комментарием, чтобы не сливать `} /// ...` в одну строку. Отступ (только пробелы после
    // '\n') строкой не считается — лишний перевод не дублируется.
    const std::string_view body = m_ectx.m_ctx.source().output_body(m_ectx.m_out);
    if (!outputAtLineStart(body)) {
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, "\n");
    }
...
// C++-вызов проверки свободного места на стеке перед вызовом защищаемой функции
// (атрибут @[stack_check@]). Пустая строка - функция не защищена или режим off.
...
std::string ExprEmitter::stackCheckExpr(const CallExpr& n) {
    ...
    // Защищаемые функции собраны при эмиссии их объявлений (DeclEmitter -> m_stackCheck).
    const auto it = m_ectx.m_stackCheck.find(std::string(n.m_callee->text()));
    if (it == m_ectx.m_stackCheck.end()) {
        return {};
    }
    const StackCheckGuard& g = it->second;
    if (g.size > 0) {
        return "trust::stack_check::check_overflow(" + std::to_string(g.size) + ")";
    }
    // size==0: @[stack_check@] без аргумента → check_stack_limit() (free >= m_stack_limit + reserve).
    return "trust::stack_check::check_stack_limit()";
}

// %trust_stack_check_set_limit(f1, f2, ...) -> trust::stack_check::set_limit({&c_f1, &c_f2})
// %trust_stack_check_set_limit(f1, f2, ...) -> trust::stack_check::set_limit({&c_f1, &c_f2})
// Резолвит имена функций в адреса; неизвестное имя - диагностика (см. resolveStackCheckAddresses).
...
void ExprEmitter::emitSetLimitFunctions(const CallExpr& n) {
    ...
    if (n.m_args) {
        for (const auto& arg : *n.m_args) {
            names.push_back(arg ? std::string(arg->text()) : std::string{});
        }
    }
    const auto addrs = m_ectx.resolveStackCheckAddresses(names);
    m_ectx.m_ctx.source().output_append(m_ectx.m_out, "trust::stack_check::set_limit({");
    for (size_t i = 0; i < addrs.size(); ++i) {
        if (i) {
            m_ectx.m_ctx.source().output_append(m_ectx.m_out, ", ");
        }
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, addrs[i]);
    }
    m_ectx.m_ctx.source().output_append(m_ectx.m_out, "})");
}

// Обработка ОТДЕЛЬНЫХ функций контроля стека (явные вызовы в коде, `%`-префикс). Это РЕАЛЬНЫЕ
// C++-функции рантайма (trust::stack_check::check_overflow/check_stack_limit/set_reserve/get_reserve/
// get_limit/set_limit), вызываемые непосредственно в коде TrustLang.
bool ExprEmitter::handleStackCheckNative(const CallExpr& n) {
    if (!n.m_callee) {
        return false;
    }
    ...
    if (name == "%trust_stack_check") {
        ...
        if (n.m_args && !n.m_args->empty()) {
            ...
            m_driver.emitExpr((*n.m_args)[0].get());
            m_ectx.m_ctx.source().output_append(m_ectx.m_out, ")");
        } else {
...
void ExprEmitter::visit_CallExpr(const CallExpr& n) {
    ...
    if (n.m_callee) {
        ...
        // Отдельные функции контроля стека (%trust_stack_check*, %trust_stack_check_set_limit) -
        // явные вызовы в коде.
        if (handleStackCheckNative(n)) {
            return;
        }
    }
    ...
    // Контроль переполнения стека: если callee - защищаемая функция (@[stack_check]*), оборачиваем
    // вызов в comma-выражение `(check(), call)`, чтобы вставка работала и в выражении, и в statement.
    const std::string guard = stackCheckExpr(n);
    if (!guard.empty()) {
        m_driver.m_type.recordRequiredInclude("@trust/stack_check.hpp");
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, "(");
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, guard);
        m_ectx.m_ctx.source().output_append(m_ectx.m_out, ", ");
    }
    // Конструктор НАТИВНОГО класса: `MyStr(...)` - callee - имя класса, зарегистрированного как
    // нативный класс (ClassDecl, Group::kNativeClass). Эмитим нативное C++-имя `std::string(...)`
    // (без мэнглинга `c_MyStr`, т.к. C++-struct НЕ генерируется) + инклуд класса on-use.
    if (n.m_callee && n.m_callee->kind() == ParserToken::Kind::Ident) {
        const auto tid = m_ectx.m_ctx.types().findType(n.m_callee->text());
        if (tid && m_ectx.m_ctx.types().isNativeClassType(*tid)) {
            const std::string_view cpp = m_ectx.m_ctx.types().nativeClassCppName(*tid);
            if (!cpp.empty()) {
                m_driver.m_type.recordUsedType(*tid); // инклуд @[include] on-use
                if (!guard.empty()) {
