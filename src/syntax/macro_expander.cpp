...
SequenceType expandMacros(const TermPtr& macro, MacroArgsType& args, Parser& parser, MapperRange callRange) {
    ...
    for (int i = 0; i < seq.size(); i++) {
        ...
        if (seq[i]->getTermID() == TermID::MACRO_TOSTR) {
            ...
            for (auto& elem : iter->second) {
                // Клон при каждой подстановке: один и тот же токен аргумента может быть подставлен
                // несколько раз (напр. имя функции в объявлении и в рекурсивном вызове @$name(...));
                // совместное использование одного TermPtr ломает разрешение типов параметров.
