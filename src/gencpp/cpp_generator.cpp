...
struct NodeCollector : AstVisitor {
    ...
    void visit(const Program* n) override {
        for (const auto& i : n->items) i->accept(this);
    }
    void visit(const FuncDecl* n) override { if (n->body) n->body->accept(this); }
    void visit(const BlockStmt* n) override { for (const auto& it : n->body) dispatch_block(it); }
    void visit(const IfStmt* n) override {
        if (n->condition) n->condition->accept(this);
        for (const auto& it : n->then_body) dispatch_block(it);
        if (n->else_if) n->else_if->accept(this);
        if (n->else_block) n->else_block->accept(this);
    }
    void visit(const WhileStmt* n) override {
        if (n->condition) n->condition->accept(this);
        for (const auto& it : n->body) dispatch_block(it);
        if (!n->else_body.empty()) {
            for (const auto& it : n->else_body) dispatch_block(it);
        }
    }
    ...
    void visit(const TryCatchStmt* n) override {
        for (const auto& it : n->try_body) dispatch_block(it);
        if (n->catch_block) visit(n->catch_block.get());
    }
    void visit(const CatchBlock* n) override { for (const auto& it : n->body) dispatch_block(it); }
    void visit(const MatchingStmt* n) override {
        types.insert(n->token_kind());
        if (n->expression) n->expression->accept(this);
        for (const auto& c : n->cases) { c->accept(this); }
        for (const auto& it : n->else_body) dispatch_block(it);
    }
    ...
    void visit(const VarDecl* n) override { types.insert(n->token_kind()); if (n->init) n->init->accept(this); }
    void visit(const AssignmentStmt* n) override {
        types.insert(n->token_kind());
        if (n->target) n->target->accept(this);
        if (n->value) n->value->accept(this);
    }
    void visit(const ReturnStmt* n) override { types.insert(n->token_kind()); if (n->value) n->value->accept(this); }
    void visit(const ExprStmt* n) override { types.insert(n->token_kind()); if (n->expr) n->expr->accept(this); }
    void visit(const ThrowStmt* n) override { types.insert(n->token_kind()); if (n->value) n->value->accept(this); }
    void visit(const BreakStmt* n) override { types.insert(n->token_kind()); (void)n; }
    void visit(const ContinueStmt* n) override { types.insert(n->token_kind()); (void)n; }
    void visit(const WhileElseBlock* n) override { for (const auto& it : n->body) dispatch_block(it); }
    void visit(const BinaryOp* n) override { types.insert(n->token_kind()); if (n->left) n->left->accept(this); if (n->right) n->right->accept(this); }
    void visit(const CallExpr* n) override { types.insert(n->token_kind()); for (const auto& a : n->args) if (a) a->accept(this); }
    void visit(const MemberAccess* n) override { types.insert(n->token_kind()); if (n->object) n->object->accept(this); }
    void visit(const ArrayAccess* n) override { types.insert(n->token_kind()); if (n->array) n->array->accept(this); if (n->index) n->index->accept(this); }
    void visit(const ArrayInit* n) override { types.insert(n->token_kind()); for (const auto& e : n->elements) if (e) e->accept(this); }
    void visit(const CastExpr* n) override { types.insert(n->token_kind()); if (n->expr) n->expr->accept(this); }
    void visit(const RefMakeExpr* n) override { types.insert(n->token_kind()); if (n->arg) n->arg->accept(this); }
    void visit(const RefTakeExpr* n) override { types.insert(n->token_kind()); if (n->arg) n->arg->accept(this); }
...
static std::string torch_dtype_str(TypeKind t) {
    switch (t) {
        case TypeKind::Int: return "torch::kInt32";
        case TypeKind::Bool: return "torch::kBool";
        default: return "torch::kInt32";
    }
}

CppGenerator::CppGenerator(GeneratorOptions opts)
    : indent_level_(0), opts_(std::move(opts)) {}
...
TypeKind CppGenerator::get_expr_type(const Expr* e) const {
    ...
    if (e) {
        if (dynamic_cast<const IntLiteral*>(e)) return TypeKind::Int;
        if (dynamic_cast<const StringLiteral*>(e)) return TypeKind::String;
        if (const auto* cast = dynamic_cast<const CastExpr*>(e)) return cast->target_type.kind;
    }
    return TypeKind::Int;
}

std::string CppGenerator::generate(const Program* program) {
    out_.str(""); out_.clear(); indent_level_ = 0;
...
void CppGenerator::visit(const FuncDecl* node) {
    ...
    for (size_t i = 0; i < node->params.size(); ++i) {
        ...
        if (i + 1 < node->params.size()) out_ << ", ";
    }
    out_ << ") {\n";
    indent_level_++;
    if (node->body) {
        for (const auto& bitem : node->body->body) dispatch_block_item(bitem);
    }
    indent_level_--;
    out_ << "}\n\n";
}
...
void CppGenerator::visit(const WhileStmt* node) {
    ...
    if (!node->else_body.empty()) {
        ...
        out_ << else_label << ":;\n";
        indent_level_++;
        for (const auto& bitem : node->else_body) dispatch_block_item(bitem);
        indent_level_--;
        indent();
        out_ << "goto " << end_label << ";\n";
        indent();
        ...
        indent();
        out_ << "do {\n";
        for (const auto& bitem : node->body) dispatch_block_item(bitem);
        indent_level_--;
        indent();
        out_ << "} while (";
        if (node->condition) node->condition->accept(this);
...
void CppGenerator::visit(const TryCatchStmt* node) {
    ...
    out_ << "try {\n";
    indent_level_++;
    for (const auto& bitem : node->try_body) dispatch_block_item(bitem);
    indent_level_--;
    indent();
    out_ << "} ";
    if (node->catch_block) {
...
void CppGenerator::build_matching_chain(Expr* expr,
                                        const std::vector<std::unique_ptr<MatchingCase>>& cases,
                                        const BlockBody& else_body,
                                        size_t idx,
                                        bool is_first) {
    if (idx >= cases.size()) {
        if (!else_body.empty()) {
            ...
            indent_level_++;
            for (const auto& bitem : else_body) dispatch_block_item(bitem);
            indent_level_--;
            indent();
            out_ << "}\n";
        } else if (!is_first) {
    ...
    if (is_first) indent(); else out_ << " else ";
    ...
    out_ << ")) {\n";
    indent_level_++;
    for (const auto& bitem : current_case->body) dispatch_block_item(bitem);
    indent_level_--;
    indent();
    out_ << "}";
...
void CppGenerator::visit(const ReturnStmt* node) {
    indent();
    out_ << "return";
    if (node->value) { out_ << " "; node->value->accept(this); }
    out_ << ";\n";
}
...
void CppGenerator::visit_if_stmt(const IfStmt* node, bool is_primary) {
    if (is_primary) indent();
    out_ << "if (";
    if (node->condition) node->condition->accept(this);
    out_ << ") {\n";
    indent_level_++;
    for (const auto& bitem : node->then_body) dispatch_block_item(bitem);
    indent_level_--;
    if (node->else_if) {
        indent();
        out_ << "} else ";
        ...
        out_ << "} else {\n";
        indent_level_++;
        for (const auto& bitem : node->else_block->body) dispatch_block_item(bitem);
        indent_level_--;
        indent(); out_ << "}\n";
    } else {
        indent(); out_ << "}\n";
...
void CppGenerator::visit(const BinaryOp* node) {
    out_ << "("; node->left->accept(this);
    out_ << " " << bin_op_to_str(node->op) << " ";
    node->right->accept(this); out_ << ")";
}

void CppGenerator::visit(const VarRef* node) { out_ << node->name; }
void CppGenerator::visit(const IntLiteral* node) { out_ << node->value; }
void CppGenerator::visit(const StringLiteral* node) {
    out_ << "\"" << StringUtils::escape(node->value) << "\"";
}
...
void CppGenerator::visit(const StructDecl* node) {
    ...
    for (const auto& method : node->methods) {
        if (auto* fd = static_cast<const FuncDecl*>(method.get())) {
            ...
            indent_level_++;
            if (fd->body) {
                for (const auto& bitem : fd->body->body) dispatch_block_item(bitem);
            }
            indent_level_--;
            indent();
            out_ << "}\n\n";
...
std::string CppGenerator::bin_op_to_str(BinOp op) const {
    return BinOpUtils::to_string(op);
}
