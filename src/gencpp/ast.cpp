...
BinaryOp::BinaryOp(BinOp o, std::unique_ptr<Expr> l, std::unique_ptr<Expr> r)
: op(o)
, left(std::move(l))
, right(std::move(r)) {
}
VarRef::VarRef(std::string n)
: name(std::move(n)) {
}
...
EnumLiteral::EnumLiteral(std::string en, std::string mn)
: enum_name(std::move(en))
, member_name(std::move(mn)) {
}
MemberAccess::MemberAccess(std::unique_ptr<Expr> obj, std::string f)
: object(std::move(obj))
, field(std::move(f)) {
}
ArrayAccess::ArrayAccess(std::unique_ptr<Expr> arr, std::unique_ptr<Expr> idx)
: array(std::move(arr))
, index(std::move(idx)) {
}
ArrayInit::ArrayInit(std::vector<std::unique_ptr<Expr>> elems, TypeKind et)
: elements(std::move(elems))
, element_type(et) {
}
CastExpr::CastExpr(std::unique_ptr<Expr> e, TypeInfo tt)
: expr(std::move(e))
, target_type(std::move(tt)) {
}
RefMakeExpr::RefMakeExpr(std::unique_ptr<Expr> a)
: arg(std::move(a)) {
}
RefTakeExpr::RefTakeExpr(std::unique_ptr<Expr> a)
: arg(std::move(a)) {
}
...
AssignmentStmt::AssignmentStmt(std::unique_ptr<Expr> t, std::unique_ptr<Expr> v)
: target(std::move(t))
, value(std::move(v)) {
}
ReturnStmt::ReturnStmt(std::unique_ptr<Expr> v)
: value(std::move(v)) {
}
ExprStmt::ExprStmt(std::unique_ptr<Expr> e)
: expr(std::move(e)) {
}
BlockStmt::BlockStmt(BlockBody b)
: body(std::move(b)) {
}
IfStmt::IfStmt(std::unique_ptr<Expr> cond, BlockBody tb, std::unique_ptr<IfStmt> elif, std::unique_ptr<BlockStmt> eb)
: condition(std::move(cond))
, then_body(std::move(tb))
, else_if(std::move(elif))
, else_block(std::move(eb)) {
}
WhileElseBlock::WhileElseBlock(BlockBody b)
: body(std::move(b)) {
}
WhileStmt::WhileStmt(std::unique_ptr<Expr> cond, BlockBody b, BlockBody eb)
: condition(std::move(cond))
, body(std::move(b))
, else_body(std::move(eb)) {
}
WhileStmt::WhileStmt(std::unique_ptr<Expr> cond, BlockBody b)
: condition(std::move(cond))
, body(std::move(b)) {
}
DoWhileStmt::DoWhileStmt(BlockBody b, std::unique_ptr<Expr> cond)
: body(std::move(b))
, condition(std::move(cond)) {
}
BreakStmt::BreakStmt() = default;
ContinueStmt::ContinueStmt() = default;
CatchBlock::CatchBlock(TypeInfo t, std::string n, BlockBody b)
: exception_type(std::move(t))
, var_name(std::move(n))
, body(std::move(b)) {
}
TryCatchStmt::TryCatchStmt(BlockBody tb, std::unique_ptr<CatchBlock> cb)
: try_body(std::move(tb))
, catch_block(std::move(cb)) {
}
ThrowStmt::ThrowStmt(std::unique_ptr<Expr> v)
: value(std::move(v)) {
}
MatchingCase::MatchingCase(std::unique_ptr<Expr> p, BlockBody b)
: pattern(std::move(p))
, body(std::move(b)) {
}
...
EnumMember::EnumMember(std::string n, std::unique_ptr<int> v)
: name(std::move(n))
, value(std::move(v)) {
}
StructField::StructField(std::string n, TypeInfo t, std::unique_ptr<Expr> i)
: name(std::move(n))
, type(std::move(t))
, init(std::move(i)) {
}
EnumDecl::EnumDecl(std::string n, std::vector<std::unique_ptr<EnumMember>> m)
: name(std::move(n))
, members(std::move(m)) {
}
...
VarDecl::VarDecl(std::string n, std::unique_ptr<Expr> i)
: name(std::move(n))
, var_type()
, init(std::move(i))
...
Program::Program(std::vector<std::unique_ptr<Decl>> i)
: items(std::move(i)) {
}
