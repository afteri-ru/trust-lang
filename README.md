...
The project has two build modes selected by the CMake build type:

- **Dev build** (`-DCMAKE_BUILD_TYPE=Debug`, or no explicit type) - the default. The embedded
  version `TRUST_VERSION` carries the git hash (`0.6.0-<git_hash>`) so every dev build is
  traceable to a concrete commit. No distribution archive / Debian package is produced, and the
  VS Code VSIX is not part of the default build (`TRUST_BUILD_VSIX` defaults to `OFF` in dev).
- **Release build** (`-DCMAKE_BUILD_TYPE=Release`) - optimized build for distribution. The
  embedded version is the clean release number (`0.6.0`, no git hash). The distribution
  archive (`package`), the Debian package (`deb`) and the VS Code VSIX (`vsix-trust-lang`) are
...
A **release build** requires a git state that exactly corresponds to the release version:
  - the checked-out branch is a **release branch** matching `VERSION` (`MAJOR.MINOR` or `MAJOR`):
    `release/<MAJOR>.<MINOR>.X` (e.g. `release/0.6.X`) or `release/<MAJOR>.X` (e.g. `release/0.X`),
    or HEAD is detached on the release tag (e.g. a CI build of the tag);
  - **HEAD is exactly on the *annotated* release tag `v<X.Y.Z>`** named by `VERSION` (e.g. `v0.6.0`)
    - `git describe --exact-match --tags HEAD` == `v<X.Y.Z>` and the tag is annotated;
  - the working tree is **fully clean** (`git status --porcelain` empty: no uncommitted /
    staged / untracked changes);
  - `CHANGELOG.md` carries a `[Release v<X.Y.Z>]` entry equal to `VERSION`.
Any other git state (e.g. `main`, dev/feature branches, a release branch whose number differs from
`VERSION`, or any state where HEAD is not exactly on the annotated release tag) is a **configuration
error**: release binaries embed the clean release number, which must correspond to the exact source
they are built from. This is enforced at configure time (`cmake/release_checks.cmake`); each error
message ends with the concrete `git` command that fixes the situation.

The artifact targets that embed `VERSION` / the git identity (`package`, `deb`, VSIX) are additionally
built only from a **fully committed tree** (no uncommitted / staged / untracked changes): an error in
a release build and a warning when they are force-enabled on a dev build
(`cmake/require_clean_tree.cmake`).

In the VS Code tasks (`.vscode/tasks.json`) configuration is a **separate** step from
build/test. Both configure tasks use the single `_build/` directory - the active build type
is the one configured last:

```sh
# configure tasks:  "cmake: configure (dev)" / "cmake: configure (release)"
cmake -B _build -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Debug     # dev
cmake -B _build -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release   # release
# build/test tasks ("cmake: build (full)", "cmake: build test binaries",
# "test: all ...") never re-run configure - they use the pre-configured _build/
cmake --build _build
```

To build or test a **release** from VS Code, run `cmake: configure (release)` first (it
reconfigures `_build` as Release), then the same build/test tasks operate on the release
configuration in `_build/`.
...
- **package/** - Distribution archive and Debian package tests (`test/package/`):
  ...
  the `unit/lit/vscode` suites pass (fixture `package_deps`). They are registered
  **only** in release builds (default `TRUST_BUILD_PACKAGE=ON` when
  `CMAKE_BUILD_TYPE=Release`, otherwise `OFF`).
...
KeywordSigil: remove      # '@' handling for keywords: remove | add | none (default remove)
```
...
#### `KeywordSigil` — normalize the `@` sigil on keywords

`KeywordSigil: remove|add|none` (`.trust-format` key; CLI `--keyword-sigil=<mode>`,
default `remove`) controls how the formatter normalizes the `@` sigil on every
recognized keyword (macro from the effective keywords list):

- `remove` — strip a leading `@` (`@return` → `return`, `@func` → `func`);
- `add` — ensure a leading `@` (`if` → `@if`);
- `none` — leave the sigil as written in the source.

## Behavioral compiler flags (`-f<name>` / `-fno-<name>`)

These are compiler (transpiler) flags, **not** diagnostics (no `-W` form):

- `-fcomments` / `-fno-comments` — insert documentation comments from the source
  `.src` into the generated C++ (default: on). `-fno-comments` suppresses `///`
  doc comments, the `// else` try-branch marker, etc.
- `-fsourcemap` / `-fno-sourcemap` — embed the source map and the export table in
  the generated C++ (default: on). The `.src_map` file is always written next to
  the `.cppt`; the flag only controls the `#embed "<name>.src_map"` block and the
...
The `package` target produces a gzipped tar archive for installation/distribution. It is
available (and built automatically as part of `cmake --build _build`) **only in release
builds** - the option `TRUST_BUILD_PACKAGE` defaults to `ON` for `CMAKE_BUILD_TYPE=Release`
and to `OFF` otherwise:

```sh
cmake -B _build -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release
cmake --build _build --target package
```

A release archive records the git hash in its manifest, so it can only be produced from a
**fully committed** working tree: building `package` with uncommitted changes to tracked files
is an error in a release build (and only a warning when the package is force-enabled on a dev
build). Also, a release configure is allowed only on the matching release branch/tag
(see "Build modes").

Without the option neither the `package`/`deb` targets nor the
`package_target_test`/`deb_package_test` CTest suites exist, so a dev build
(`Debug`) does not produce or test them. When the option is enabled, the archive is
built automatically as part of the default build (`cmake --build _build`), independently
of running the tests (test binaries and `ctest` are not required). The archive (and the
`.vsix` package, when built) are placed in a dedicated `_build/dist/` directory. The
archive name encodes the build attributes; on a release build the version is the clean
release number (no git hash):

```
trust-lang-<version>-<os>-<arch>.tar.gz
```

where `<version>` is the release number (e.g. `0.6.0`), `<os>` derives from
`CMAKE_SYSTEM_NAME` (e.g. `linux`) and `<arch>` from
...
automatic installation is built alongside the archive. Both are built and tested
only in release builds (release archive/package only; a dev build must not produce
