...
- **Запрещено читать, писать или выполнять поиск** в каталоге [`docs/`](docs/), так как он **не является частью кодовой базы**. 

## Подпроекты

| Подпроект | Описание |
|-----------|----------|
| [`diag`](src/diag/) | Диагностика — сообщения, форматирование, source manager, CLI-опции |
| [`parser`](src/parser/) | Токенизация (Flex) и парсинг (Bison), генерация токенов из `tokens.def`, MMProc, AST |
| [`types`](include/types/) | Система типов — `TypeKind`, `TypeInfo`, совместимость, user-defined типы |
| [`runtime`](include/runtime/) | Среда выполнения и стандартная библиотека — type-erased контейнеры, исключения, подсчёт ссылок, builtin-функции (`print`, `len`, `assert`, математика, коллекции) |
| [`cli`](src/pipeline/) | Командная строка — драйвер полного pipeline (lex → parse → analyze → optimize → codegen) |
| [`jit`](src/solver/) | JIT-компиляция — LLVM ORC JIT, разрешение символов, память выполнения |
| [`lsp`](src/lsp/) | Language Server — autocomplete, go-to-definition, hover-диагностика, форматирование |
| [`repl`](src/runtime/) | Интерактивная оболочка — read-eval-print, автодополнение, инкрементальная JIT |
