...
if [ "$WEB_PORT" = "no" ]; then
    ...
    # локальную песочницу). Примеры читаются trust-lsp напрямую из корневого ../examples
    # (встраиваются в фрагмент при генерации, отдельные копии в content/ не нужны).
    echo "[playground_run] generating playground fragment (local server-url=$PORT)"
    mkdir -p fragments
    ../_build/trust-lsp --html "../examples/hello.src" --examples-dir "../examples" \
