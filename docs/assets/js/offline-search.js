  ...
  $(document).ready(function () {
    ...
    $.ajax($searchInput.data('offline-search-index-json-src')).then((data) => {
      idx = lunr(function () {
        ...
        // Enable searching of both the Russian (Cyrillic) and English content.
        // lunr's default pipeline is English-only and uses an ASCII-only
        // trimmer, so Cyrillic tokens would otherwise be discarded entirely.
        // Requires lunr.stemmer.support.js + lunr.ru.js + lunr.multi.js to be
        // loaded before this script (see layouts/partials/head.html).
