...
tags: [architecture, naming, compiler]
weight: 20
---
...
{{< unreleased why="описанный здесь манглинг (внутренние имена с суффиксами `$`/`::`, сигнатуры функций со словарём аргументов, интерпретатор) относится к устаревшей архитектуре. Текущая реализация маппит trust-имена в C++-идентификаторы с префиксом `c_` (utils::name_to_cpp), юникод транслитерируется, нативные имена `%` сохраняются без изменения (см. include/transpiler/MEMORY.md)" />}}
...
что позволяет обращатся к объектам TrustLang из внешних библиотек и приложений на языке реализации (C/C++) 
...
В [расширенном синатксисе](/ru/docs/operators/block/#embed) с исходным кодом языка реализации
можно взаимодействовать с другим кодом программы за счет обращения к локальным объектам или макросам изнутри блока кода. 

Это допускается как для обычных переменных, так и для имен функций, реализованных в TrustLang.
...
Каждая функция TrustLang имеет две сигнатуры, первая (*основная*) принимает все аргументы в виде словаря,
...
```python
    ...
    # Определение функции в TrustLang
