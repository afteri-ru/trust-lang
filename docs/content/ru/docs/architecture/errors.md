...
tags: [architecture, errors, runtime]
weight: 30
---
...
{{< unreleased why="описанная здесь схема обработки ошибок (макросы LOG_ERROR, опция --nlc-error-limit, классы :IntAny/:IntPlus/:IntMinus) относится к устаревшей архитектуре `nlc`. Текущая реализация использует компонент diag (Severity::Error/Warning/... и накопительные отчёты в pipeline), а опции задаются через -W<name> (см. README.md)" />}}
...
### Возврат знечений
Для обработки [перываний потока выполения](/ru/docs/operators/throw/), используются отдельные С++ классы,
