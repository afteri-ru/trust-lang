...
{{< unreleased why="системные свойства объектов `__name__`, `__type__`, `__module__`, `__str__` и т.п. в текущей реализации-транспиляторе не реализованы (нет интерпретируемой объектной модели; данные компилируются в C++). Нижеследующее описание относится к устаревшей архитектуре" />}}

Несмотря на то, что не все объекты *TrustLang* являются классами с точки зрения [реализованной концепции ООП](/ru/docs/types/class/), 
...
В файле dsl.nlp находится набор макросов, которые расширяют базовый синтаксис *TrustLang*, основанный на правилах,
...
### Операторы
- **match(cond)** - Оператор [оценки выражения](/ru/docs/operators/match/)
...
### Прерывания
- **return** - Выход из функции (аналог break по имени текущей функции **@__FUNCTION__**):
  - **@return;** - Void-форма (**@__FUNCTION__ ++** → `return;`)
  - **@return <значение>;** - С возвратом значения (**@__FUNCTION__ ++ @$value ++** → `return value;`).
    Значение - один rvalue (имя, литерал, строка, вызов или словарь `(a, b,)` в скобках).

- **break** - Выход из ближайшего цикла или из именованного блока:
  - **@break;** - Выход из цикла (**++** → `break;`)
  - **@break <метка>;** - Выход из именованного блока (метка - один идентификатор, БЕЗ `::`: **<метка> ++** → `goto <метка>_break;`)

- **continue** - Переход к началу цикла или именованного блока:
  - **@continue;** - К началу цикла (**-+** → `continue;`)
  - **@continue <метка>;** - К началу именованного блока (**<метка> -+** → `goto <метка>_continue;`)
...
При работе парсера *TrustLang* автоматически формируются несколько зарезервированных макросов, часть из которых соответствуют макросам препроцессора у С/С++.
