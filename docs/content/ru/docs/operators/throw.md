...
Последовательность команд [(блок кода)](/ru/docs/operators/block/) можно прервать в любом месте с помощью специальных операторов.
...
Именованные [блоки кода](/ru/docs/operators/block/) могут выступать в качестве локальных меток для передачи потока выполнения.
...
ns:: { # Пространство имен ns
    ...
    func_name(val):Int32 := {  
        ...
        # Возврат из функции
        func_name ++ 0 ++;
        # Или @return 0; с использованием DSL
...
в любом [блоке кода](/ru/docs/operators/block/) и возвращают результат, но их еще и можно *"ловить"*. 
...
типе возвращаемого значения. Если именованные блоки кода [возвращают значение](/ru/docs/operators/throw/#return), 
...
Блоки кода с перехватом прерываний, совместно с оператором [оценки выражения](/ru/docs/operators/match/), 
...
> Сопрограмма (англ. coroutine) - программный модуль, особым образом организованный для обеспечения взаимодействия с другими модулями 
> по принципу кооперативной многозадачности: модуль приостанавливается в определённой точке, 
> **сохраняя полное состояние (включая стек вызовов и счётчик команд), и передаёт управление другому**, 
> тот, в свою очередь, выполняет задачу и передаёт управление обратно, сохраняя свои стек и счётчик. 
...
{{< unreleased why="сопрограммы и связанные с ними макросы/операторы (@co_yield/@co_await/@co_return, coro::when_all/when_any, :Async) в текущей версии НЕ реализованы; в include/trust/dsl.src соответствующие макросы закомментированы, а конструкциям присвоен Kind=Unimplemented" />}}

```python

co_return   -> ++ или ++ ... ++ (обычный оператор возврата, но в коде С++ заменяется на co_return)
co_yield    -> [++] или [++ ... ++] только внутри корутины 
co_await    -> [*] ожидание корутины (задачи) только внутри корутины

coro::when_all ->  [**]( ...  ) ожидание всех задач
coro::when_any ->  [***]( ...  ) ожидание любой из задач

coro::latch
coro::sync_wait(awaitable)  блокирующее ожидание задачи
coro::task<@ ... @>
coro::generator<@ ... @>
