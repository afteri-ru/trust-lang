...
{{< unreleased why="описанный здесь менеджер контекста (`**()` / `@with`), операторы захвата ссылок (`*`, `&`, `&?`, `&&`), межпотоковая синхронизация и тип `:Class`/`:Thread` в текущей версии НЕ реализованы" />}}
