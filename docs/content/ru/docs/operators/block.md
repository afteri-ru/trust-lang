...
{{< unreleased why="блок-скоупинг пространств имён `ns:: ( ... )`/`name ( ... )` в текущей версии работает иначе, чем показано в примере (см. docs/syntax/namespace). Нативный блок `{% ... %}` (embed) - реализован" />}}
...
Существуют блоки кода, которые перехватывают потоки выполнения, с помощью которых реализуются [возврат и обработка ошибок](/ru/docs/operators/throw/).
