...
Диапазон (`1..10`), а также массивы и тензоры описаны в разделе
[Диапазоны, массивы и тензоры](containers/).

### Поток (:Thread)

{{< unreleased why="тип :Thread (запуск функции в отдельном потоке, методы start()/join()) в текущей версии НЕ реализован: `:Thread()` даёт ошибку 'unable to generate C++ type Any'. Многопоточность и межпотоковая синхронизация не реализованы" />}}
...
{{< unreleased why="тип :System и выполнение системных команд (в т.ч. через обратные кавычки `` `cmd` ``) в текущей версии НЕ реализованы" />}}

Для доступа к различным функциям операционной системы *TrustLang* предоставляет специальный тип данных **:System**.
