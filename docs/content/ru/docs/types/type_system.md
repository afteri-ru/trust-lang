...
*TrustLang* имеет закрытую систему типов. т.е. любой новый тип данных, который создается пользователем,
обязательно должен быть наследником одного из уже существующих, т.е. расширять один из них. 

{{< unreleased why="данный документ содержит концептуальные заметки и черновиковые пометки автора (в т.ч. со знаками '???'), а не описание реализованного синтаксиса. Приведённая иерархия типов, описание чистых функций, корутин и Thread/Async/ThreadPool не соответствуют текущей реализации (корутины, потоки, чистые функции `:-`/`::-` не реализованы; см. docs/types/funcs)" />}}
...
в TrustLang с динамоческой типизации минимальным прототипом класса будет 
...
С помощью обобщений и [шаблонов](/ru/docs/syntax/generics/) можно писать код, который может работать с любым совместимым типом данных. 
...
{{< unreleased why="описанный ниже синтаксис чистых функций (pure() :- ( )), корутин (coro() := []()( )), Thread/Async/ThreadPool не реализован в текущей версии" />}}
