...
url: docs/syntax/namespace
weight: 20
---

*TrustLang* поддерживает пространства имен, разделителем в которых, как и в С++, выступает двойное двоеточие "**::**".

Область имен может быть указана как для отдельного индетификатора, так и для целого [блока кода](/ru/docs/4_operators/block/).
...
Для использования пространства имен в [блоке кода](/ru/docs/4_operators/block/), его нужно указать перед открывающейся фигурной скобкой.
В таком именованном блоке кода завершающие "**::**" указывать не обязательно.

{{< unreleased why="префикс `@::` для глобальных статических переменных не реализован. Проверено: глобальный квалификатор `::var := N` работает, но вложенный блок-скоупинг `ns ( ... )` с доступом через `ns::var`/`::ns::var` в текущей версии НЕ работает так, как показано (объявленная внутри переменная не видна снаружи как ns::var)" />}}
...
Пространство имен в *TrustLang* поддерживает одновременно с [модульной структурой кода](/ru/docs/syntax/modules/) как в языках Java и Python
...
В *TrustLang* не применяется декорирование (манглинг) имен, основанный на типах аргументов функций.

Тем не менее, при создания уникальных идентификаторов для методов [классов](/ru/docs/types/class/), *TrustLang* использует подход, похожий на применяемый в языке Python. 
...
{{< unreleased why="данный синтаксис создания метода класса через блок пространства имён `NewClass:: ( ... )` и через полное имя `NewClass::method() := ...` не соответствует текущей реализации классов (см. docs/types/class)" />}}
