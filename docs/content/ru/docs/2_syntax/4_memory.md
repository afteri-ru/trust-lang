...
- символ захвата/разименования ("**\***") - создается ссылочная переменная без контроля совместного досутпа
    (для использования только в текущем потоке или для [асинхронного программирования](/ru/docs/4_operators/async/#async)).
...
- символ *простой* ссылки для совместного доступа ("**&?**") - создается ссылочная переменная без объекта синхронизации доступа,
     для работы с которой требуется использовать [групповой](/ru/docs/4_operators/with/#light-link) захват ссылок.
...
При захвате ссылки результат сохраняется в *локальную (автоматическую)* переменную и может быть индивидуальным или [групповым](/ru/docs/4_operators/with/).
...
Для захвата переменных используются следующие операторы: 
 ...
 - '**\*\*( ... )**' - [групповой](/ru/docs/4_operators/with/) захват ссылок в *локальные (автоматические)* переменные
...
над переменными можно захватить объект в локальную переменую или использовать [менеджер контекста](/ru/docs/4_operators/with/).
