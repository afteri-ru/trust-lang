...
В *TrustLang* реализовано безопасное автоматическое управление памятью без сборщика мусора.

Автоматическое освобождение памяти реализовано за счет принципа [RAII](https://en.wikipedia.org/wiki/Resource_acquisition_is_initialization)
...
а захват и освобождение объекта синхронизации при разыменовании ссылки выполняются *автоматически*.
...
Запускаемые примеры работы со ссылками — в песочнице:
[перестановка ссылок](/ru/playground/?file=references_swap), [совместное владение](/ru/playground/?file=references_shared),
[трассировка ссылок](/ru/playground/?file=references_reftrace).

## Терминология {#rules}
### Переменные и их время жизни {#lifetime}
Выделение и освобождение памяти происходит автоматически при создании/удалении
переменных (на стеке или в куче); вручную память выделить нельзя.

По времени жизни переменных в *TrustLang* может разделить на два класса:
...
- [*Постоянные*](/ru/docs/syntax/naming/#sigil) переменные (объекты) - это глобальные переменные, функции и типы данных,
    ...
    где они были определёны (после выхода из текущей области видимости).
- *Автоматические* [*(локальные или временные)*](/ru/docs/syntax/naming/#sigil) переменные, это аргументы функций и переменные в выражениях, 
    ...
    Временные объекты доступные только изнутри того лексического контекста, в котором они были определёны.
...
- символ захвата/разименования ("**\***") - создается ссылочная переменная без контроля совместного досутпа
    (для использования только в текущем потоке или для [асинхронного программирования](/ru/docs/operators/async/#async)).
...
- символ *простой* ссылки для совместного доступа ("**&?**") - создается ссылочная переменная без объекта синхронизации доступа,
     для работы с которой требуется использовать [групповой](/ru/docs/operators/with/#light-link) захват ссылок.
...
При захвате ссылки результат сохраняется в *локальную (автоматическую)* переменную и может быть индивидуальным или [групповым](/ru/docs/operators/with/).
...
Для захвата переменных используются следующие операторы: 
 ...
 - '**\*\*( ... )**' - [групповой](/ru/docs/operators/with/) захват ссылок в *локальные (автоматические)* переменные
...
над переменными можно захватить объект в локальную переменую или использовать [менеджер контекста](/ru/docs/operators/with/).

### Пример

Работающие примеры управления ссылками — в песочнице:
[перестановка ссылок](/ru/playground/?file=references_swap), [совместное владение](/ru/playground/?file=references_shared), [трассировка ссылок](/ru/playground/?file=references_reftrace).
