...
{{< unreleased why="пример использует нереализованные в текущей версии конструкции: оператор `:=`, препроцессорную декларацию `@( ... )`, итераторы `@iter/@curr/@next`, тип `:Thread` с методами start()/join() (потоки не реализованы). Проверено: `@iter`/`@curr`/`@next` дают 'undefined name', `:Thread()` не компилируется" />}}
