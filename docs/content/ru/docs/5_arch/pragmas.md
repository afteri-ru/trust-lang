...
### Управление диагностикой компилятора {#diag}
@\_\_PRAGMA_DIAG\_\_ -         
```
...
## Управление опциями компилятора {#options}
@\_\_OPTION\_\_, @\_\_OPTION_PUSH\_\_, @\_\_OPTION_POP\_\_

Макросы для временного изменения severity диагностических опций компилятора из исходного кода:

```
    @__OPTION_PUSH__;
    @__OPTION__("macro-redefined", "ignore");
    # ... определения макросов, переопределения разрешены ...
    @__OPTION_POP__;
```

- `@__OPTION_PUSH__` — сохраняет текущий набор опций.
- `@__OPTION__("<имя_опции>", "<значение>")` — устанавливает severity опции. Значение — одно из: `fatal`, `error`, `warning`, `remark`, `note`, `ignore`.
- `@__OPTION_POP__` — восстанавливает предыдущий набор опций.

Опция `macro-redefined` управляет поведением при переопределении макроса: по умолчанию `fatal` (запрещено), `ignore` — разрешает переопределение без диагностики.

## Управление макросами {#syntax}
