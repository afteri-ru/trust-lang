...
{{< unreleased why="именованные аргументы в форматной строке (name = Строка) НЕ реализованы: пример с именованными плейсхолдерами не компилируется (ошибка в сгенерированном C++ std::format). Реализован формат с позиционными плейсхолдерами (args) → std::format без именованных аргументов (см. playground/string_format.src)" />}}
