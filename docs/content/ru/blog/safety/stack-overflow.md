...
## Статус интеграции в TrustLang

Функциональность контроля переполнения стека встроена в компилятор [TrustLang](/ru/docs/overview/) как
контроль **на этапе кодогенерации** (роль clang-плагина берёт транспилятор, а рантайм-примитивы
`trust::stack_check` — публичный рантайм-заголовок `trust/stack_check.hpp`).

- **Атрибуты функций** (вставка проверки ПЕРЕД каждым вызовом помеченной функции):
  - `@[stack_check@]` — без аргумента = **limit**: перед каждым вызовом `check_stack_limit()`
    (свободно ≥ `m_stack_limit` из секции `.stack_sizes` + резерв);
  - `@[stack_check(N)@]` — целое `N`: `check_overflow(N)` (явный размер свободного места).
- **Отдельные функции** (реальные C++-функции рантайма, явные вызовы в коде, `%`-префикс):
  - `%trust_stack_check(N)` — `check_overflow(N)` (free ≥ N + reserve);
  - `%trust_stack_check()` — `check_stack_limit()` (free ≥ m_stack_limit + reserve);
  - `%trust_stack_check_set_reserve(N)` — `set_reserve(N)` (мин. резерв для всех функций);
  - `%trust_stack_check_get_reserve()` — `get_reserve()`;
  - `%trust_stack_check_get_limit()` — `get_limit()` (текущий `m_stack_limit`);
  - `%trust_stack_check_set_limit(f1, f2, ...)` — `set_limit({&c_f1, &c_f2})` (ограничить `m_stack_limit`
    перечисленными функциями; пусто = все функции `.stack_sizes`).
- **Резерв `reserve`** (мин. размер стека для создания исключения, default 8192) ВСЕГДА добавляется
  к проверкам: `stack_check` = `N + reserve`, `stack_check_limit` = `m_stack_limit + reserve`.
- **Управление**:
  - `--stack-check=<off|explicit|recursion|auto>` (default `explicit`), `-Wstack-check=<mode>`,
    `-Wno-stack-check`; из кода — `@__OPTION__("stack-check", "...")` + `@__OPTION_PUSH__/POP__`;
  - минимальный резерв — `--stack-check-reserve=<bytes>` (default 8192) / `-Wstack-check-reserve=` /
    `@__OPTION__("stack-check-reserve", "...")`;
  - `--stack-check-functions=<names>` — ограничить `m_stack_limit` перечисленными функциями (пусто = все);
  - `recursion` — диагностика `-Wstack-check-infer` для незащищённых рекурсивных функций;
    `auto` — авто-маркировка рекурсивных функций.
- Сгенерированная программа компилируется с `-fstack-size-section` и `-lpthread`; TLS `info`
  определяется в сгенерированном entry `_main.cppt`.
