...
Определение функции происходит с помощью операторов [создания объектов](/en/docs/ops/create/),
имя функции должно соответствовать [правилам именования](/en/docs/syntax/naming/),
...
Телом функции должен быть [блок кода](/en/docs/ops/block/), который располагается в пространстве имен определения этой функции.
...
Синтаксис *TrustLang* поддерживает два типа функций: обычные и чистые функции.
...
Но зато лямбда функцию можно передавать как аругмент в другие функции, например при создании [итератора](/en/docs/types/iter/).
```
  _()::={ ... };
...
Перегрузка функций по типам аргументов *TrustLang* отсуствует, поэтому нельзя создать несколько функций с однинаковым именем, но разными типами аругментов. 
...
Новая функция должна иметь аргументы, [совместимые](/en/docs/syntax/) с типом аргумента первоначальной функцией.
