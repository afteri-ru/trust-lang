...
*TrustLang* является языком с динамической типизацией и явное указание типа переменной не обязательно 
...
*TrustLang* имеет закрытую иерархическую систему типов, т.е. любой новый тип данных, которые создает пользователь,
...
За счет закрытой иерархии типов реализуется и парадигма [обобщённого программирования (generics)](/en/docs/syntax/).

Каждый тип данных имеет *краткое имя* и функцию - конструктор.

*Краткое имя* типа всегда начинается с символа двоеточия "**:**" 
и определяется в [глобальной области видимости](/en/docs/syntax/namespace/).

Функция - конструктора типа, имеет тоже самое *краткое имя* типа, но без символа двоеточия "**:**",
и как и любая функция может располагаться в пользовательском [пространстве имен](/en/docs/syntax/namespace/).
...
но сами функции должны располагаться в разных областях [пространства имен](/en/docs/syntax/namespace/).
...
в противном случае будет вызвана функция согласно алгоритму [разрешения имен (name lookup). ](/en/docs/syntax/naming/).
...
Создание типов происходит согласно [правилам синтаксиса](/en/docs/ops/create/).
Имя типа всегда начинается с символа двоеточия "**:**" и определяется в [глобальной области видимости](/en/docs/syntax/namespace/).
Создание сложных [типов (классов)](/en/docs/types/class/), может располагаться в пользовательском [пространстве имен](/en/docs/syntax/namespace/).

, в том числе и [конструктор классов](/en/docs/types/class/), 
---
