...
Именование объектов основано на [правилах](/en/docs/syntax/naming/) и выбирается из расчёта удобства для программиста. 
...
Кроме этого, все [квалификаторы](/en/docs/syntax/naming/#sigil) объектов переносятся в конец имени,
...
Внутреннее имя уникально только в рамках одного [модуля](/en/docs/syntax/modules/), но внутренние имена в разных модулях могут пересекаться.
...
а у имени типа [квалификатор](/en/docs/syntax/naming/#sigil) заменяется тремя символами '**$**', т.е. 'type **$$$**'.
...
В [расширенном синатксисе](/en/docs/ops/block/#embed) с исходным кодом языка реализации
можно взаимодействовать с другим кодом программы за счет обращения к локальным объектам или макросам изнутри блока кода. 

Это досупкается как для обычных переменных, так и для имен функций, реализованных в TrustLang,
