...
Несмотря на то, что не все объекты *TrustLang* являются классами с точки зрения [реализованной концепции ООП](/en/docs/types/class/), 
...
### Операторы
- **match(cond)** - Оператор [оценки выражения](/en/docs/ops/match/)
...
- **return** - Выход из функции (аналог break по имени текущей функции **@__FUNCTION__**):
  ...
  - **@return <значение>;** - С возвратом значения (**@__FUNCTION__ ++ @$value ++** → `return value;`).
    Значение - один rvalue (имя, литерал, строка, вызов или кортеж `(a, b,)` в скобках).
