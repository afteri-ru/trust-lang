...
В *TrustLang* реализовано автоматическое управление памятью без сборщика мусора.
...
- Управление временем жизни объекта включает в себя не только управлением памятью, 
    ...
    - *легкая* ссылка может быть создана для совместного доступа ("**&?**"), 
        но её захват и синхронизация доступа к ней возможен только при [групповом](/en/docs/ops/with/) захвате ссылок.
...
- Захват *слабой* ссылки может быть индивидуальным или [групповым](/en/docs/ops/with/) 
...
Для захвата ссылок используются операторы: 
 ...
 - '**\*\*( ... )**' - [групповой](/en/docs/ops/with/) захват ссылок в *локальные (контролируемые)* переменные
...
над переменными можно захватить объект в локальную переменую или использовать [менеджер контекста](/en/docs/ops/with/).
...
Для того, чтобы однократно захватить объект(ы) синхронизации для выполнения сразу нескольких действий 
над переменными используется [менеджер контекста](/en/docs/ops/with/).
