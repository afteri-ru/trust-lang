...
Последовательность команд [(блок кода)](/en/docs/ops/block/) можно прервать в любом месте с помощью специальных операторов.
...
Именованные [блоки кода](/en/docs/ops/block/) могут выступать в качестве локальных меток для передачи потока выполнения.
Если перед оператором прерывания указать [пространство имен](/en/docs/syntax/namespace/), 
...
Тоже самое с использованием [макросов DSL](/en/docs/syntax/dsl/).:
```python
    b1 := 1;
...
и `abort()` для *отрицательного*, что отражено в [DSL](/en/docs/syntax/dsl/).
...
в любом [блоке кода](/en/docs/ops/block/) и возвращают результат, но их еще и можно *"ловить"*. 
...
Блоки кода с перехватом прерываний, совместно с оператором [оценки выражения](/en/docs/ops/match/), 
