...
Одиночный оператор захвата [ссылки и синхронизации доступа](/en/docs/syntax/memory/) к объекту '**\***' или '**\*(...)**'
...
Или таже самая запись с использованием [макросов DSL](/en/docs/syntax/dsl/):
```python
...
Освобождение ресурсов (вызов [деструктора у объекта](/en/docs/types/class/)) происходит до попадания в ветку *else*, если она присутствует.
