...
```
Или тоже самое, но с использованием [макросов из модуля dsl](/en/docs/syntax/dsl/):
```python
    @while(cond) {
        ...
    } @else {
...
```python
    summa := 0;
    dict := (1,2,3,4,5,);
    [ dict ] <-> { # Цикл пока есть данные
        # Результат оператора распаковки словаря: первый элемент перемещается в item,
        # суффикс `dict...` связывает оставшиеся элементы (мутация источника в цикле)
        item, dict... := ... dict; 
