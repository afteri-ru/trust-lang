...
## [Unreleased]
- **Trust-конструкции (пред/пост-условия и утверждения) + единая опция `--solver` (= `-Wsolver=<mode>`, X-макрос).**
  DSL-мнемоники `trust_pre( <expr> )`/`trust_post( <expr> )`/`trust_assert( <expr> )` ↔ сырые маркеры
  `@( ... @)`/`@< ... @>`/`@{ ... @}`; обработка по `--solver=[ignore|warning|error|assert|export|calculate]`
  (по умолчанию `warning`): `warning` - предупреждение о наличии любого условия, `error` -
  ошибка, `assert` - рантайм-проверки через единый **интринсик `trust::intrinsic_assert`**
  (см. `types/intrinsics.hpp`; `@assert`/`@verify` и trust-условия раскрываются в него и
  разворачиваются в `if(!(cond)) trust::trust__abort__(file, line, "<cond>", backtrace)` на этапе
  генерации - единый источник, без копипаста тела макроса), `export`/`calculate` - не реализованы
  (диагностика). Анализатор: резолв имён в условиях через стек скоупов; в пред-условии имя функции
  (возвращаемое значение) - ошибка, в пост-условии - допустимо; тип с trust-условиями не выводится
  автоматически (только явная аннотация). Тип/функция с условиями - семантически отличный тип
  (бит trust в TypeKind, раздельное интернирование). Общий помощник `sourceLocation(map, range)`
  для «файл:строка» (парсер `@__FILE_NAME__`/`@__FILE_LINE__` + транспилятор).
- **Мнемонические команды DSL `@def`/`@func` + подсказки в IDE.** Добавлены встроенные DSL-макросы
  `def` (префикс-маркер; голое использование раскрывается в `@__PRAGMA_ERROR__` и завершается ошибкой,
  документирующий комментарий со списком зарезервированных подкоманд служит подсказкой), `func` и
  `def func` (определение функции без оператора `:=`), обе формы Void и с типом возврата:
  `@func имя( args ) { тело }` → `имя( args ) := { тело }`; `@func имя( args ): тип { тело }` →
  `имя( args ): тип := { тело }` (тип - несколько токенов, в т.ч. `Tuple(Int32, Int32)`).
  **Макропроцессор:** локальный шаблон с аргументами `$name ( ... )` извлекает и имя (`@$name`), и
  скобочную группу (`( ... )` → `@$...`) - `ExtractArgs`/`CheckMacro`/`MakeMacroId`; `:` (разделитель
  типа) и `{` (терминатор сигнатуры, граница тела) допускаются в сигнатуре (исключены из
  `deny_chars_from_macro`; `{`/`}` внутри `@@ ... @@` в лексере не трогают brace-стек - новый флаг
  `Scanner::m_inMacroSeq`, паритет `m_macro_count` ненадёжен т.к. его мутирует парсер); вариативная
  группа `$...` (тип возврата) захватывает токены до следующего терма сигнатуры (`{`), пропуская
  вложенные скобки; **групповая адресация `@$1...`/`@$2...`** (лексер `@$"[1-9]"..."` → MACRO_ARGUMENT)
  различает скобочные аргументы и тип возврата; добавлена адресация **элемента группы по номеру
  `@$G.N`** (`@$1.2`, `@$2.1` и т.п.) и **количества `@$G.#`/`@$#G`** (для перебора; стиль единообразен:
  `@$G` + суффикс `...`/`.N`/`.#`, группы многоцифровые);
  имя макроса может начинаться с `@` (`@@ @def func ... @@`, `@` исключён из deny);
  `CompareMacroName` сопоставляет `:`/`{`/`}` дословно;
  `ExpandString` - двухпроходная подстановка (`@`-ключи до bare), чтобы `$name` не портил `@$name`.
  **НЕотключаемые ошибки макросов** (без `-W`-опций): операторный `$...` (последний) + call-группа
  (оператор поглощает аргументы) и несколько `$...` в сигнатуре (хвостовой + `$...` в скобках любого
  вида, коллизия `@$...`) - макрос принципиально нераскрываем; сообщение подсказывает добавить
  терм-маркер (напр. `{`) после последнего `$...`.
  **LSP:** completion-элементы макросов несут `documentation` (док из `BuiltinCatalog::dslMacroDocs()`
  и `SymbolInfo::documentation`; пустая документация символа не затирает док из каталога), подсказка
  показывается при начале ввода `@def`. Имена `def`/`func` зарезервированы DSL (как `main`/`module`/`return`).
...
- **`extern "C"` только для forward-деклараций функций:** определения функций (с телом) - обычные
  ...
  [dcl.link]); это убирает warning `-Wreturn-type-c-linkage` у С++ компилятора для `auto`-возврата кортежа у
