// trust/trusted-cpp-sync.hpp - MULTI-THREADED reference type (AccessShared<V, Policy>).
//
...
// (which policy, shared vs exclusive, timeout). Here AccessShared owns the policy (Policy template
...
//   AccessMutex        - plain exclusive mutex;
//   AccessRwMutex      - timed read/write mutex (shared read-only / exclusive write).
...
// Shared synchronized storage: the value plus the access lock that guards it. The lock mechanism is
// the policy by default (Impl = void) or the requested implementation class (3rd reftype argument)
// when Impl is given; the policy then remains a semantic tag (naming / read-vs-write mode).
// default_timeout - пер-объектный таймаут детектора задаётся аргументом конструктора
// AccessShared(value, timeout) (в атрибуте reftype таймаут НЕ указывается) либо nullopt = глобальный.
template <typename V, typename Policy, typename Impl = void>
struct AccessData {
    using Lock = std::conditional_t<std::is_void_v<Impl>, Policy, Impl>;
    V value;
    mutable Lock lock;
    std::optional<SyncTimeoutType> default_timeout;
    explicit AccessData(V v)
    : value(std::move(v)) {}
    AccessData(V v, SyncTimeoutType t)
    : value(std::move(v))
    , default_timeout(t) {}
};
...
// forwards the release to the owning lock (policy or implementation class).
template <typename V, typename Policy, typename Impl = void>
class SyncRelease : public detail::LockRelease {
  public:
    explicit SyncRelease(std::shared_ptr<AccessData<V, Policy, Impl>> data, bool shared_mode)
    : data_(std::move(data))
    , shared_mode_(shared_mode) {}

    void unlock() noexcept override {
        if (shared_mode_) {
            data_->lock.unlock_shared();
        } else {
            data_->lock.unlock();
        }
    }
  ...
  private:
    std::shared_ptr<AccessData<V, Policy, Impl>> data_;
    bool shared_mode_;
};
} // namespace detail

// ----------------------------------------------------------------------------
// Встроенные политики доступа. Все предоставляют ЕДИНЫЙ интерфейс,
// используемый AccessShared:
 ...
 * shared read-only lock_const() and AccessSingleThread for per-thread confinement.
 *
 ...
 */
template <typename V, typename Policy = AccessMutex, typename Impl = void>
class AccessShared {
  public:
    using ValueType = V;
    using WeakType = std::weak_ptr<detail::AccessData<V, Policy, Impl>>;
    using SharedType = std::shared_ptr<detail::AccessData<V, Policy, Impl>>;

    AccessShared()
    : data_(nullptr) {}

    explicit AccessShared(const V& val)
    : data_(std::make_shared<detail::AccessData<V, Policy, Impl>>(val)) {}

    explicit AccessShared(V&& val)
    : data_(std::make_shared<detail::AccessData<V, Policy, Impl>>(std::move(val))) {}

    /// Конструктор с пер-объектным таймаутом детектора (перекрывает глобальный).
    AccessShared(const V& val, SyncTimeoutType default_timeout)
    : data_(std::make_shared<detail::AccessData<V, Policy, Impl>>(val, default_timeout)) {}

    AccessShared(V&& val, SyncTimeoutType default_timeout)
    : data_(std::make_shared<detail::AccessData<V, Policy, Impl>>(std::move(val), default_timeout)) {}

    AccessShared(const AccessShared&) = default;
    AccessShared& operator=(const AccessShared&) = default;

    AccessShared(AccessShared&&) noexcept = default;
    AccessShared& operator=(AccessShared&&) noexcept = default;
    ...
    static Locker<V, false> make_auto(const SharedType& shared, const SyncTimeoutType& timeout = block_timeout()) {
        ...
        if (!shared->lock.try_lock(timeout)) {
    ...
    static Locker<V, true> make_auto_const(const SharedType& shared, const SyncTimeoutType& timeout = block_timeout()) {
        ...
        if (!shared->lock.try_lock_shared(timeout)) {
            detail::trusted_cpp_error("deadlock detected: lock timeout");
        }
    ...
    [[nodiscard]] std::optional<Locker<V, false>> try_lock(const SyncTimeoutType& timeout = SyncTimeoutNoWait) const {
        ...
        if (!data_->lock.try_lock(timeout)) {
            return std::nullopt;
        }
    ...
    [[nodiscard]] std::optional<Locker<V, true>> try_lock_const(const SyncTimeoutType& timeout = SyncTimeoutNoWait) const {
        ...
        if (!data_->lock.try_lock_shared(timeout)) {
            return std::nullopt;
        }
        return make_guard_const(data_, /*shared_mode=*/true);
    }

    AccessShared& set(const V& value) {
        auto guard = lock();
        *guard = value;
        return *this;
    }
    AccessShared& set(const V& value, const SyncTimeoutType& timeout) {
        auto guard = lock(timeout);
        *guard = value;
        return *this;
    }

    AccessShared& set(V&& value) {
        auto guard = lock();
        *guard = std::move(value);
        return *this;
    }
    AccessShared& set(V&& value, const SyncTimeoutType& timeout) {
        auto guard = lock(timeout);
        *guard = std::move(value);
        return *this;
    }
    ...
    [[nodiscard]] Weak<AccessShared> weak() const { return Weak<AccessShared>(*this); }
  ...
  private:
    ...
    static Locker<V, false> make_guard(const SharedType& shared, bool shared_mode) {
        ...
        auto release = std::make_shared<detail::SyncRelease<V, Policy, Impl>>(shared, shared_mode);
...
// -- Deadlock-free ordered multi-acquisition (used by the `with` construct) --------------------
// Locks two synchronized references in a DETERMINISTIC order (by control-block address), so that
// callers using opposite argument orders cannot deadlock. Guards are released in reverse order by
// the returned tuple's destructor. Heterogeneous value types / policies are allowed.
template <typename VA, typename PA, typename IA, typename VB, typename PB, typename IB>
[[nodiscard]] std::tuple<Locker<VA, false>, Locker<VB, false>> lock_all(const AccessShared<VA, PA, IA>& a, const AccessShared<VB, PB, IB>& b) {
    if (std::less<const void*>{}(a.get_shared().get(), b.get_shared().get())) {
        auto ga = a.lock();
        auto gb = b.lock();
        return std::tuple<Locker<VA, false>, Locker<VB, false>>(std::move(ga), std::move(gb));
    }
    auto gb = b.lock();
    auto ga = a.lock();
    return std::tuple<Locker<VA, false>, Locker<VB, false>>(std::move(ga), std::move(gb));
