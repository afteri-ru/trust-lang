...
inline std::string parseDriverArgs(std::span<std::string> args, std::span<const DriverOption> table, const ApplyOptionFn& apply,
                                   ...
                                   std::vector<std::string>* analysis_passthrough = nullptr, std::vector<std::string>* run_args = nullptr) {
    ...
    for (std::size_t i = 1; i < args.size(); ++i) {
        ...
        if (positional_only) {
            if (input_file.empty()) {
                ...
                run_args->push_back(tok); // --run: аргументы программы после входного файла
            } else {
        ...
        if (tok.size() >= 2 && tok[0] == '-' && tok[1] == '-') {
            ...
            if (tok[1] == 'W') {
                // Команды справки `-Whelp` / `-Whelp-dsl` / `-Whelp-predef-macros` /
                // `-Whelp-check-areas`: ранний сигнал для пропуска проверки входного файла
                // (печать - в приложении; `-Whelp` печатается через Options, макро-справки - в trust.cpp).
                diag_args.push_back(tok);
                if (tok == "-Whelp" || tok == "-Whelp-dsl" || tok == "-Whelp-predef-macros" || tok == "-Whelp-check-areas") {
...
inline std::string driverHelp(std::span<const DriverOption> table) {
    ...
    out += "Macro help: use `-Whelp-dsl` (DSL macros) or `-Whelp-predef-macros` (predefined @__...__ macros).\n";
    out += "Areas: use `-Whelp-check-areas` (syntactic areas for @__CHECK_AREA__).\n";
...
inline std::vector<DriverOption> commonAnalysisOptions(int solverModeId, int keywordsId, int solverLoopUnrollId, int stackCheckId = -1,
                                                       int stackCheckReserveId = -1, int stackCheckFunctionsId = -1) {
    std::vector<DriverOption> opts = {
    ...
    if (stackCheckId >= 0) {
        opts.push_back({stackCheckId, "stack-check", "", CliOpt::Value, "mode",
                        "Stack overflow protection mode: off | explicit | recursion | auto (default: explicit). "
                        "Not a diagnostic; toggled also via -Wstack-check=<mode> and @__OPTION__(\"stack-check\", ...).\n"
                        "  off       - disabled (attributes/macros are no-ops)\n"
                        "  explicit  - checks only for functions marked @[stack_check@]/@[stack_check(N)@] and %trust_stack_check functions\n"
                        "  recursion - like explicit + -Wstack-check-infer diagnostic for unguarded recursive functions\n"
                        "  auto      - like explicit + auto-mark recursive functions",
                        CliCategory::ProjectSpecific});
    }
    if (stackCheckReserveId >= 0) {
        opts.push_back({stackCheckReserveId, "stack-check-reserve", "", CliOpt::Value, "bytes",
                        "Minimum stack reserve (bytes) for exception handling, added to all stack checks; "
                        "empty = runtime default (8192). Also settable via @__OPTION__(\"stack-check-reserve\", ...).",
                        CliCategory::ProjectSpecific});
    }
    if (stackCheckFunctionsId >= 0) {
        opts.push_back({stackCheckFunctionsId, "stack-check-functions", "", CliOpt::Value, "names",
                        "Comma-separated function names limiting the automatic stack limit (m_stack_limit from "
                        ".stack_sizes); empty = all functions. Also settable via @__OPTION__(\"stack-check-functions\", ...).",
                        CliCategory::ProjectSpecific});
    }
    return opts;
}
