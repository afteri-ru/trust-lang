...
enum class DeclResult {
    ...
    Overloaded, ///< Имя уже занято функцией с ОТЛИЧАЮЩЕЙСЯ сигнатурой: набор перегрузок расширен.
    Duplicate, ///< Имя уже занято: точный дубль / конфликт kinds (func vs var).
};
...
struct Symbol {
    ...
    /// Набор СИГНАТУР перегруженного имени (каждая - интернированный FunctionTypeData TypeId,
    /// возможно с флагами - читать через structuralType). Пусто - имя НЕ перегружено, `type` -
    /// единственная сигнатура. Непусто - имя адресует несколько перегрузок: `type` хранит лишь
    /// ПРЕДСТАВИТЕЛЯ (первую сигнатуру) и НЕ должен использоваться как «сигнатура имени»;
    /// единственный корректный путь - разрешение перегрузки резолвером
    /// (types/overload_resolve.hpp). Набор хранится на привязке имени, а не как отдельный
    /// тип (см. .tasklog: перегрузка - привязка имени, не тип-категория).
...
class SymbolTable {
  public:
    ...
    /// ИНВАРИАНТ: `sym` НЕ модифицируется (копия кладётся в таблицу) - после вызова хук
    /// `onDeclare(sym)` видит полноценный Symbol (имя/тип/decl), а не moved-from.
    DeclResult declareOrComplete(const Symbol& sym);
