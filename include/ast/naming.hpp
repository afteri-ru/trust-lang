...
class Ident : public std::string {
  public:
    ...
    // ── Нормализация ──
    // Нормализованный идентификатор не содержит ^, @-макросов, field ('.'),
    // временных имён ($) и специальных имён ($0, $$, $*, $^, $1..$N).
    ...
    // \rel\mod  + base_dir=/proj  + sys_dir=/sys → /proj/rel/mod
    // \\abs\mod + base_dir=...    + sys_dir=...  → /abs/mod
    // \\\sys\mod + base_dir=...   + sys_dir=/sys → /sys/mod
    ...
    // /proj/rel/mod + base_dir=/proj → \rel\mod
    // /abs/mod      + base_dir=/proj → \\abs\mod
    static Ident path_to_module_name(const std::filesystem::path& path, const std::filesystem::path& base_dir);
};
