...
3. **MMProc (Macros & Module Processor)** — `mmproc.hpp` / `mmproc.cpp`
    - Конвертирует `LexemeSequence` → `ASTNodeSequence`, заполняя `TokenInfo`.
    - Конкатенирует строковые литералы одного типа (STRWIDE, STRCHAR, RAW).
    - Формирует ошибки через `diag()` для неподдерживаемых токенов (MACRO, MODULE).
    - **Планируется:** раскрытие макросов, загрузка внешних модулей.
...
5. **Парсер (Bison)** — грамматический разбор на основе `parser.y`.
   - Принимает `ASTNodeSequence` после MMProc
