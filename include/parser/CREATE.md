...
# Создание объектов и копирование значений
...
Все операторы создания объектов или присвоения значения уже существуюим имеют одинаковый синтаксис.
` <имя объекта> <оператор> <данные> ; `, где:

- <имя объекта> - один из идентификаторов, которые описаны в `NAMING.md`
- <оператор> - один из операторов создания объекта или изменения данных.
- <данные> - данные для инициализации или копирования


### Создания объектов {#create}

Для создания объектов в TrustLang используется операторы:

- `::=` - всегда создает новый объект (переменную, функцю, тип или класс). Если объект с таким именем уже был определён ранее, то новый объект будет его перекрывать, но нельзя создать два объекта с одинаковым именем в одной области видимости блоке кода.  

- `:=` - используется для присвоения нового значения уже существующей переменной или создает новую, если переменная с указанным имененм отсуствует. ~~*Не применим для создания новых типов, функций или методов классов.*~~   
...
Этот же оператор применятеся и для переопределления винутральных функций или методов классов, когда предыдущая релаизаиця не *удаляется*, а сохранятеся в виртулаьной табилце переопределных имен. ?????
