...
- **Диагностика + quickfix** - публикация всех severity (`publishDiagnostics`) и `textDocument/codeAction` (kind `quickfix` по `fixits`); конверсии Severity→LSP/range - в общем `sourcemap/protocol.hpp`.
