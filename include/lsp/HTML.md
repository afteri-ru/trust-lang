...
Фрагмент - самодостаточен: встроены `<style>`, разметка, конфиг и glue-JS.
Внешней остаётся только библиотека Monaco (`--monaco-url`).

Цвета фрагмента - CSS-переменные `--tpl-*`. Дефолты на `.tpl-pg` ссылаются на
общую с сайтом палитру `--site-*` (`var(--site-*, <fallback>)`), определённую
сайтом в `docs/assets/scss/_styles_project.scss` - так песочница и сайт имеют
единую цветовую гамму, задаваемую в одном месте.
...
`height: calc(100dvh - 11rem)` (минимум 540px, фолбэк `100vh`), `.tpl-row` и
`.tpl-editor` - `flex: 1 1 auto` / `flex: 1` с `min-height: 0`, поэтому панели
редакторов заполняют доступное пространство. `.tpl-log` (фиксированная высота,
`flex: none`) лежит под горизонтальным сплиттером `#tpl-split-h`, который
размещён ВНЕ `.tpl-row` (между строкой панелей и логом), поэтому перетаскивание
меняет высоту окна лога.
