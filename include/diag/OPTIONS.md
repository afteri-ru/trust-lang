...
2. **Опции драйвера (CLI)** — `include/driver/cli.hpp` (компонент `pipeline`).
...
| `-Whelp-dsl` | справка по макросам загруженного DSL (имена + первая строка документирующего комментария) |
| `-Whelp-predef-macros` | справка по предопределённым макросам `@__...__` (имя + описание) |
...
**Макро-справка (`-Whelp-dsl`, `-Whelp-predef-macros`)** — команды в стиле `-Whelp`, но НЕ
относятся к диагностикам и НЕ управляются `Options` как наборы: diag — «лист» и не знает про
DSL/макросы. `Options` лишь фиксирует тему запроса через `Options::helpTopic()`
(`HelpTopic::{DslMacros, PredefMacros}`; `-Whelp` → `HelpTopic::Diagnostics`), а печать списков
выполняется в приложении (`trust.cpp`):
- `-Whelp-dsl` — имена макросов загруженного DSL (`trust/dsl.src` или `--dsl`) из реестра
  `Context::macroDefs()` + первая строка документирующего комментария (`##`/`##<`), если он есть;
- `-Whelp-predef-macros` — предопределённые макросы вида `@__...__` (value + context + прагмы)
  из x-macro таблиц (`predef_macro_x.hpp`/`pragma_macro_x.hpp`) с описанием.
...
| `-Whelp` | команда справки | справка по диагностикам анализа кода | `Options::help_topic_` (в `Options::parse_argv`; `-Whelp` → `HelpTopic::Diagnostics`) | `Options::printHelp()` |
| `-Whelp-dsl` | команда справки | справка по макросам загруженного DSL | `Options::help_topic_` (в `Options::parse_argv`; → `HelpTopic::DslMacros`) | печать в `trust.cpp` (`printDslMacroHelp`) |
| `-Whelp-predef-macros` | команда справки | справка по предопределённым макросам `@__...__` | `Options::help_topic_` (в `Options::parse_argv`; → `HelpTopic::PredefMacros`) | печать в `trust.cpp` (`printPredefMacroHelp`) |
| `-Werror`/`-Wno-error` | диагностики | глобальный переключатель warning→error | `Options::m_werror` | — |

**`--help` и `-Whelp`-команды — разные флаги в разных слоях, никогда не смешиваются.** У них даже
разные механизмы справки (таблица драйвера vs реестр диагностик / печать в приложении).

### Два флага для команд справки `-Whelp*` (ранний сигнал vs собственно флаг)

Каждая команда справки (`-Whelp`, `-Whelp-dsl`, `-Whelp-predef-macros`) обрабатывается на двух уровнях:

- `ParseResult::diag_help_requested` — **ранний драйверный сигнал**: выставляется в
  `parseDriverArgs` (детект литералов `-Whelp`/`-Whelp-dsl`/`-Whelp-predef-macros`). Нужен
  `Pipeline::parseArgs`/`trust.cpp`, чтобы пропустить проверку обязательности входного файла
  **до** создания `Context`/`Options`.
- `Options::helpRequested()`/`helpTopic()` — **собственно флаг/тема справки**: выставляется в
  `Options::parse_argv`. Печать идёт по теме: `HelpTopic::Diagnostics` → `Options::printHelp()`;
  `HelpTopic::DslMacros`/`PredefMacros` → макро-справка в `trust.cpp`
  (`if (ctx.opts().helpRequested()) switch (ctx.opts().helpTopic()) ...`).
