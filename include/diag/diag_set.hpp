...
// Формат строки TRUST_FLAG_SET: M(EnumName, "cli", "default_value", DiagGroup, "help").
// Значение по умолчанию value-флага идёт СРАЗУ ПОСЛЕ имени опции, описание - последним аргументом.
// Вспомогательные макросы вариадические, чтобы быть совместимыми со списками, не задающими
// значение по умолчанию (пустая строка "" = нет значения по умолчанию).
#define TRUST_FL_ENUM(name, ...) name,
#define TRUST_FL_DESC(name, cli, defval, grp, help, ...) {cli, help, grp},
