...
/// Вид защиты стека вызова функции (из атрибута @[stack_check@]),
/// собираемый при эмиссии объявления/определения функции (см. DeclEmitter).
/// size == 0 → check_stack_limit() (free >= m_stack_limit + reserve); size > 0 → check_overflow(size).
struct StackCheckGuard {
    long size = 0; ///< @[stack_check(N)@] → check_overflow(N); 0 = limit (check_stack_limit)
};
...
class CppEmitContext {
  public:
    ...
    /// Резолв имён функций в C++-адреса (reinterpret_cast<void*>(&c_name)) для set_include_functions
    /// (ограничение m_stack_limit перечисленными функциями). Неизвестное имя - диагностика. Возвращает
    /// список адресов только известных функций (сохраняя порядок).
    ...
    /// Имя temp приходит из узла (`JumpStmt::m_tempDecl`), счётчик не нужен (временные создаёт анализатор).
    ...
    /// Для entry-функции (`<модуль>__main__`): C++-типы параметров ТОЛЬКО (через ", "),
    /// без имён. Ими pipeline генерирует совпадающий extern в `_main.cppt` (entry эмитится
    /// в основном `.cppt`, а `main` обёртка - в отдельном файле, поэтому сигнатура нужна
    /// обоим). Пусто, если entry без параметров (`@main()`).
    std::string m_entryParams;

    /// True, если при кодогенерации эмитилась entry-функция (`<модуль>__main__`). Позволяет
    /// pipeline отличить «программу с точкой входа» от библиотеки/скрипта без `__main__`
    /// (m_entryParams пуст и для `@main()` без параметров, и для отсутствия entry вовсе).
    bool m_sawEntry = false;

    /// Однофайловый режим генерации (`-fsingle-file`), активен ТОЛЬКО для корневого модуля
    /// главного файла. Задаётся через CppTranspiler::configureSingleFile (runPipeline). В этом
    /// режиме при отсутствии entry-функции DeclEmitter оборачивает top-level операторы модуля в
    /// синтезируемую entry-функцию `<модуль>__main__` (режим «модуль-скрипт», python-подобно).
    bool m_singleFileMode = false;
    /// Имя синтезируемого implicit-entry в single-file script-режиме (`<модуль>__main__`), в точности
    /// совпадающее с ожидаемым pipeline `entry_func_name` (чтобы pipeline-обёртка
    ...
    /// Защищаемые функции (trust-имя → вид защиты стека) из атрибута @[stack_check@];
    /// собирается при эмиссии объявления/определения функции.
    mutable std::unordered_map<std::string, StackCheckGuard> m_stackCheck;

    /// Имена всех функций, эмитированных в модуле (trust-имя) - для резолва адресов
    /// `--stack-check-functions` при эмиссии include-списка в точке входа.
    mutable std::set<std::string> m_functionNames;
